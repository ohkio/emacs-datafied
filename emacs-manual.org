...
*** Displaying the Cursor
    ...
    curosr, and switches to it when you start or resume Emacs.

    On a graphical display, many more properties of the text curosr
    can be altered. To customize its color, change the ~:background~
    attribute of the face named ~cursor~. To change its chape,
    ...
    the count. You can customize the variable ~blink-curosr-blinks~ to
    control that.
...
*** Syntax of Regular Expressions
    ...
    - ~[ ... ]~ :: Is a character set, beginning with '~[~' and
      ...
      starting and eding characters with '~-~' between them. You can
...
*** Backslash in Regular Expressions
    ...
    - ~\|~ :: specifies an alternative. Two regular expressions /a/
      ...
      by trhing to match /a/, and if that fails, by trying to match
    ...
    - ~\(?: ... \)~ :: specifies a shy group that does not record
      ...
      without interfereing with the numbering of the groups that are
    ...
    - ~\_< :: matches the empty string, but only at the beginning of a
      symbol. A symbol is a seuqnce of one or more symbol-consittuent
...
*** Lax Matching During Searching
    ...
    as well as to literal tring search. The effect ceases if you
...
*** Key Sequence
    ...
    input events that is meaninful as a unit.
*** Complete Key
...
** Entering Emacs
*** Startup Screen
    When Emacs starts up, the initial fram displays a special buffer
...
*** Visiting
    Emacs obays the command ~find-file~ by /visiting/ the file: it
...
*** Wildcard
    ...
    matches any string of characters at the corrosponding position in
...
*** Primary Selection
    Under X, every timme the active region changes, Emacs saves the
...
*** Face Attributes
    ...
    height, weignt, slant, foreground and background color, and
    underling or overling.
*** Color Name
    A /color name/ is a pre-defined name, such as '~dark orange~' or
    '~medium sea green~'.
*** RGB Triplet
    An /RGB triplet/ is a string of the form '~#RRGGBB~'. Each of the
    primary color components is represented by a hexadecminal number
    between '~00~' (intencity 0) and '~FF~' (the maximum intencity).
*** Fontifies
...
*** Character Set
    In the simiplest case, the characters between the two brackets in
...
*** global-display-line-numbers-mode                  :display:buffer:window:
    :PROPERTIES:
    :Description: Doggle Display Line Numbers mode for all buffers
    :Context:  navigating
    :END:
** Searching and Replacement
...
*** isearch-highlight-regexp                                 :search:display:
    ...
    :Context:  isearch
    :END:
*** isearch-highligh-lines-matching-regexp              :search:display:line:
    :PROPERTIES:
    :Command:  M-s h l
...
*** isearch-aforward-regexp                                          :search:
    :PROPERTIES:
    :Command:  C-M-s
...
*** transpose-chars                                               :edit:char:
    ...
    useless, this tranpsoses the last two characters on the line.
...
*** blink-cursor-alist                                     :display:nav:list:
    ...
    cursor is displayed as /on-type/ when it plinks on (where
...
*** display-line-numbers-major-tick              :display:window:buffer:line:
    ...
    line. Note that major ticks take precidence over minor ticks.
...
*** isearch-yank-on-move                                 :search:region:edit:
    When this variable is customized to ~shift~, you can etend the
...
*** search-highlight                                         :search:display:
    ...
    setting this variable to ~nil~.
*** lazy-highligh-initial-delay                              :search:display:
...
*** search-nonincremental-instead                                    :search:
    ...
    customize this variable to ~nil~, typine ~RET~ will always exit
...
*** search-slow-speed                                                :search:
    Emacs provides a special display mode for slow terminals, wherby
