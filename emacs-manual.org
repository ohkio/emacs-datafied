...
*** Backslash in Regular Expressions
    ...
    - ~\_<~ :: matches the empty string, but only at the beginning of
      a symbol. A symbol is a sequence of one or more
      symbol-constituent characters. A symbol-constituent character is
      a character whose syntax is either '~\w~' or '~_~'. '~\_<~'
      matches at the beginning of the buffer only if a
      symbol-constituent character follows. As with words, the syntax
      table determines which characters are symbol constituent.
...
***** Automatic Deletion of Backups
      ...
      versions.
***** Copying vs. Renaming
      Backup files can be made by copying the old file or by renaming
      it. This makes a difference when the old file has multiple names
      (hard links). If the old file is renamed into the backup file,
      then the alternate names become names for the backup file. If
      the old file is copied instead, then the alternate names remain
      names for the file that you are editing, and the contents
      accessed by those names will be the new contents.

      The method of making a backup file may also affect the file's
      owner and group. If copying is used, these do not change, if
      renaming is used, you become the file's owner, and the file's
      group becomes the default (different operating systems have
      different defaults for groups).
      ...
      - Otherwise, if the variable ~backup-by-copying-when-linked~ is
        ...
        names, use copying.
      - Otherwise, if the variable ~backup-by-copying-when-mismatch~
        is non-nil (the default is ~t~), and renaming would change the
        file's owner or group, use copying. If you change
        ~backup-by-copying-when-mismatch~ to ~nil~, Emacs checks the
        numeric user-id of the file's owner and the numeric group-id
        of the file's group. If either is no greater than
        ~backup-by-copying-when-privileged-mismatch~, then it behaves
        as though ~backup-by-copying-when-mismatch~ is non-nil
        anyway.
      - Otherwise, renaming is the default choice.

      When a file is managed with a version control system, Emacs does
      not normally make backups in the usual way for that file. But
      committing new versions of files is similar in some ways to
      making backups. One unfortunate similarity is that these
      operations typically break hard links, disconnecting the file
      name you visited from any alternate names for the same
      file. This has nothing to do with Emacs - the version control
      system does it.
**** Customizing Saving of Files
     Normally, when a program writes a file, the operating system
     briefly caches the file's data in main memory before committing
     the data to disk. This can greatly improve performance; for
     example, when running on laptops, it can avoid a disk spin-up
     each time a file is written. However, it risks data loss if the
     operating system crashes before committing the cache to disk.

     To lessen this risk, Emacs can invoke the ~fsync~ system call
     after saving a file. Using ~fsync~ does not eliminate the risk of
     data loss, partly because many systems do not implement ~fsync~
     properly, and partly because Emacs's file-saving procedure
     typically relies also on directory updates that might not survive
     a crash even if ~fsync~ works properly.
...
**** Protection against Simultaneous Editing
     ...
     both make changes, and then both save them. If nobody is informed
     that this is happening, whichever user saves first would later
     find that their changes were lost.

     On some systems, Emacs notices immediately when the second user
     starts to change the file, and issues an immediate warning. On
     all systems, Emacs checks when you save the file, and warns if
     you are about to overwrite another user's changes. You can
     prevent loss of the other user's work by taking proper corrective
     action instead of saving the file.

     When you make the first modification in an Emacs buffer that is
     visiting a file, Emacs records that the file is locked by
     you. (It does this by creating a specially-named symbolic link
     with special contents in the same directory.) Emacs removes the
     lock when you save the changes. The idea is that the file is
     locked whenever an Emacs buffer visiting it has unsaved changes.

     When Emacs detects a collision, it asks you what to do, by
     calling the Lisp function ~ask-user-about-lock~. You can redefine
     this function for the sake of customization.

     If Emacs or the operating system crashes, this may leave behind
     lock files which are stale, so you may occasionally get warnings
     about spurious collisions.

     Note that locking works on the basis of a file name; if a file
     has multiple names, Emacs does not prevent two users from editing
     it simultaneously under different names.

     A lock file cannot be written in some circumstances, e.g., if
     ...
     collision when you try to save a file, by checking the file's
     last-modification date. If the file has changed since the last
     time Emacs visited or saved it, that implies that changes have
     been made in some other way, and will be lost if Emacs proceeds
     with saving. Emacs then displays a warning message and asks for
     confirmation before saving.
**** Shadowing Files
     A shadow file group is a set of identically-named files shared
     ...
     copying without exiting Emacs, by using the command
     ~shadow-copy-files~.

     Each shadow cluster has a name, and specifies the network address
     of a primary host (the one we copy files to), and a regular
     expression that matches the host name of all the other hosts in
     the cluster.

     To set up a shadow file group, use ~shadow-define-literal-group~
     or ~shadow-define-regexp-group~. See their documentation strings
...
*** Reverting a Buffer
    ...
    Somce kinds of buffers that are not associated with files, such as
    Dired buffers, can also be reverted. for them, reverting means
    recalculating their contents. Buffers created explicitly with ~C-x
    b~ cannot be reverted; ~revert-buffer~ reports an error if you
    try.
*** Auto Revert: Keeping buffers automatically up-to-date
**** Auto Reverting Non-File Buffers
     There are two ways to auto-revert certain non-file buffers: by
     enabling Auto Revert mode in those buffers; and by setting
     ~global-auto-revert-non-file-buffers~ to a non-nil value. The
     latter enables Auto Reverting for all types of buffers for which
     it is implemented (listed in the menu below).

     Like file buffers, non-file buffers should normally not revert
     while you are working on them, or while they contain information
     that might get lost after reverting. Therefore, they do not
     revert if they are modified. This can get tricky, because
     deciding when a non-file buffer should be marked modified is
     usually more difficult than for file buffers.

     Another tricky detail is that, for efficiency reasons, Auto
     Revert often does not try to detect all possible changes in the
     buffer, only changes that are major or easy to detect. Hence,
     enabling auto-revert for a non-file buffer does not always
     guarantee that all information is the buffer is up-to-date, and
     does not necessarily make manual reverts useless.
     ...
     Some non-file buffers can be updated reliably by file
     notification on their default directory; Dired buffers is an
     example. Such notification does not include changes to files in
     that directory, only to the directory itself.
***** Auto Reverting the Buffer Menu
      If auto-reverting of non-file buffers is enabled, the Buffer
      Menu automatically reverts every ~auto-revert-interval~ seconds,
      whether there is a need for it or not.

      If the Buffer Menu inappropriately gets marked modified, just
      revert it manually using ~g~ and auto-reverting will
      resume. However if you marked certain buffers to get deleted or
      to be displayed, you have to be careful, because reverting
      erases all marks. The fact that adding marks sets the buffer's
      modified flag prevents Auto Revert from automatically erasing
      the marks.
***** Auto Reverting Dired buffers
      Dired buffers only auto-revert when the file list of the
      buffer's main directory changes (e.g., when a new file is added
      or deleted). They do not auto-revert when information about a
      particular file changes (e.g., when the size changes) or when
      inserted subdirectories change. To be sure that all listed
      information is up to date, you have to manually revert using
      ~g~, even if auto-reverting is enabled in the Dired
      buffer. Sometimes, you might get the impression that modifying
      or saving files listed in the main directory actually does cause
      auto-reverting. This is because making changes to a file, or
      saving it, very often causes changes in the directory itself;
      for instance, through backup files or auto-save files. However
      this is not guaranteed.

      If the Dired buffer is marked modified and there are no changes
      you want to protect, then most of the time you can make
      auto-reverting resume by manually reverting the buffer using
      ~g~. There is one exception. If you flag or mark files, you can
      safely revert the buffer. This will not erase the flags or marks
      (unless the marked file has been deleted, of course). However,
      the buffer will stay modified even after reverting, and
      auto-reverting will not resume. This is because, if you flag or
      mark files, you may be working on the buffer and you might not
      want the buffer to change without warning. If you want
      auto-reverting to resume in the precense of marks and flags,
      mark the buffer non-modified using ~M-~~. However, adding,
      deleting or changing marks or flags will mark it modified
      again.

      Remote Dired buffers are currently not auto-reverted. Neither
      are Dired buffers for which you used shell wildcards or file
      arguments to list only some of the files. ~*Find*~ and
      ~*Locate*~ buffers do not auto-revert either.

      Note that auto-reverting Dired buffers may not work
      satisfactorily oon some systems.
*** Auto-Saving: Protection Against Disasters
    From time to time, Emacs automatically saves each visited file in
    a separate file, without altering the file you actually use. This
    is called auto-saving. It prevents you from losing more than a
    limited amount of work if the system crashes.

    When Emacs determines that it is time for auto-saving, it
    considers each buffer, and each is auto-saved if auto-saving is
    enabled for it and it has been changed since the last time it was
    auto-saved. Errors occurring during auto-saving are caught so that
    they do not interfere with the execution of commands you have been
    typing.
**** Auto-Save Files
     Normally, the auto-save file name is made by appending '~#~' to
     the front and rear of the visited file name. Most buffers that
     ...
     made by appending '~#~' to the front and rear of the buffer name,
     then adding digits and letters at the end of the
     uniqueness. Auto-save file names are made this way unless you
     reprogram parts of Emacs to do something different (the functions
     ~make-auto-save-file-name~ and ~auto-save-file-name-p~). The file
     name to be used for auto-saving in a buffer iss calculated when
     auto-saving is turned on in that buffer.

     When you delete a substantial part of the text in a large buffer,
     auto save turns off temporarily in that buffer. This is because
     if you deleted the text unintentionally, you might find the
     auto-save file more useful if it contains the deleted text. To
     reenable auto-saving after this happens, save the buffer with
     ~C-x C-s~, or use ~C-u 1 M-x auto-save-mode~

     If you want auto-saving to be done in the visited file rather
     than in a separate auto-save file, enable the global minor mode
     Auto Save Visited mode. In this mode, auto-saving is identical to
     explicit saving. Note that this mode is orthogonal to the Auto
     Save mode; you can enable both at the same time. However if Auto
     Save mode is active in some buffer and the obsolete
     ~auto-save-visited-file-name~ variable is set to a non-nil value,
     that buffer won't be affected by ~auto-save-visited-mode~

     A buffer's auto-save file is deleted when you save the buffer in
     its visited file. Changing the visited file name with ~C-x C-w~
     or ~set-visited-file-name~ renames any auto-save file to go with
     the new visited name.
**** Controlling Auto-Saving
     Auto-saving during idle periods accomplishes two things: first,
     it makes sure all your work is saved if you go away from the
     terminal for a while; second, it may avoid some auto-saving while
     you are actually typing.

     Emacs also does auto-saving whenever it gets a fatal error. This
     includes killing the Emacs job with a shell command such as
     '~kill %emacs~', or disconnecting a phone line or network
     connection.
**** Recovering Data from Auto-Saves
*** File Name Aliases
    Symbolic links and hard links both make it possible for several
    file names to refer to the same file. Hard links are alternate
    names that refer directly to the file; all the names are equally
    valid, and no one of them is preferred. By contrast a symbolic
    link is a kind of defined alias. More complex cases occur when
    ...
    long file name on a system that truncates long file names, or on a
    case-insensitive file system.
*** File Directories
    In verbose directory listings, Emacs adds information about the
    amoung of free space on the disk that contains the directory.
*** Comparing Files
*** Diff Mode
    Patches sometimes include trailing whitespace on modified lines,
    as unintentional and undesired change. There are two ways to deal
    with this problem. Firstly if you enable Whitespace mode in a Diff
    buffer, it automatically highlights trailing whitespace in
    modified lines. Secondly, you can use the command ~M-x
    diff-delete-trailing-whitespace~, which searches for trailing
    whitespace in the lines modified by the patch, and removes that
    whitespace in both the patch and the patched source file(s).
*** Copying, Naming and Renaming Files
    Emacs has several commands for copying, naming, and renaming
    files. All of them read two file names, /old/ (or /target/) and
...
*** File Archives
    A file whose name ends in '~.tar~' is normally a file archive made
    by the ~tar~ program.

    If Auto Compression mode is enabled, then Tar mode is used also
    for compressed archives - files with extensions '~.tgz~',
    '~.tar.Z~' and '~.tar.gz~'.

    You don't need the ~tar~ program to use Tar mode - Emacs reads the
    archives directly. However, accessing compressed archives requires
    the appropriate uncompression program.
*** Remote Files
    ...
    To carry out this request, Emacs uses a remote-login program such
    as ~ssh~. You must always specify in the file name which method to
    use - for example, ~/ssh:user@host:filename~ uses ~ssh~. When you
    specify the pseudo method '~-~' in the file name, Emacs chooses
    the method as follows:

    1. If the host name starts with '~ftp.~' (with dot), Emacs uses
       FTP.
    2. If the user name is '~ftp~' or '~anonymous~', Emacs uses FTP.
    3. If the variable ~tramp-default-method~ is set to '~ftp~', Emacs
       uses FTP.
    4. If ~ssh-agent~ is running, Emacs uses ~scp~.
    5. Otherwise, Emacs uses ~ssh~.

    You can entirely turn off the remote file name feature by setting
    the variable ~tramp-mode~ to ~nil~. You can turn off the feature
    in individual cases by quoting the file name with '~/:~'.

    Remote file access through FTP is handled by the Ange-FTP package,
    which is documented in the following. Remote file access through
    other methods is handled by the Tramp package, which has it's own
    manual.

    When the Ange-FTP package is used, Emacs logs in through FTP using
    the given /user/, if that is specified in the remote file name. If
    ...
    to a string, that string is used instead. When logging in, Emacs
    may also ask for a password.

    For performance reasons, Emacs does not make packup files for
    files accessed via FTP by default.
    ...
    handled specially. See variable
    ...
    gatewaying features, you can still use remote file names; all you
    have to do is specify the name of the gateway machine by setting
    the variable ~ange-ftp-gateway-host~, and set
    ~ange-ftp-smart-gateway~ to ~t~. Otherwise you may be able to make
    remote file names wor, but the procedure is complex. You can read
    the instructions by typing ~M-x finder-commentary <RET> ange-ftp
    <RET>~.
*** Quoted File Names
    You can /quote/ an absolute file name to prevent special
    characters and syntax in it from having their special effects. The
    way to do this is to add '~/:~' at the beginning.

    If you want to quote only special characters in the local part of
    a remote file name, you can quote just the local
    part. '~/ssh:baz:/:/foo:/bar~' refers to the file ~bar~ of
    directory ~/foo:~ on the host ~baz~.

    '~/:~' can also prevent '~' from being treated as a special
    character for a user's home directory. For example, '/:/tmp/~hack'
    revers to a file whose name is '~hack' in directory ~/tmp~.
    ...
    You can also qupte wildcard characters with '~/:~', for
    visiting. For example, ~/:/tmp/foo*bar~ visits the file
    ~/tmp/foo*bar~.
...
*** File Name Cache
    ...
    only for the duration of the Emacs session.
*** Convenience Features for Finding Files
*** Viewing Image Files
    Displaying the file as an image works only if Emacs is compiled
    with support for displaying such images.

    If Emacs was compiled with support for the ImageMagick library, it
    can use ImageMagick to render a wide variety of images.

    The Image-Dired package can also be used to view images as
    thumbnails.
*** Filesets
    If you regularly edit a certain group of files, you can define
    them as a fileset. This lets you perform certain operations, such
    as visiting, ~query-replace~, and shell commands on all the files
    at once.

    The commands ~filesets-open~, ~filesets-close~, and
    ~filesets-run-cmd~ are all available from the '~Filesets~' menu,
    where each existing fileset is represented by a submenu.

    When dealing with Version Control, there is a different concept of
    filesets: groups of files bundled together for version control
    operations. Filesets of that type are unnamed, and do not persist
    across Emacs sessions.
** Using Multiple Buffers
   Each time you visit a file, a buffer is used to hold the file's
   text. Each time you invoke Dired, a buffer is used a hold the
   ...
   ("killed") when no longer needed, either by you or by Emacs (e.g.,
   when you exit Emacs).

   Each buffer has a unique name, which can be of any length. When a
   buffer is displayed in a window, its name is shown in the mode
   line. The distinction between upper and lower case matters in
   buffer names. Most buffers are made by visiting files, and their
   names are derived from the files' names; however, you can also
   create an empty buffer with any name you want. A newly started
   Emacs has several buffers, including one named ~*scratch*~, which
   can be used for evaluating Lisp expressions and is not associated
   with any file.

   We sometimes say that a command operates on "the buffer"; this
   really means that it operates on the current buffer. When there is
   only one Emacs window, the buffer displayed in that window is
   current.

   A buffer's contents consist of a series of characters, each of
   which optionally carries a set of text properties that can specify
   more information about that character.

   Aside from its textual contents, each buffer records several pieces
   of information, such as what file it is visiting (if any), whether
   it is modified, and what major mode and minor modes are in effect.

   A buffer's size cannot be larger than some maximum, which is
   defined by the largest buffer position representable by Emacs
   integers. This is because Emacs tracks buffer positions using that
   data type. For typical 64-bit machines, this maximum buffer size is
   2^{61} - 2 bytes, or about 2 EiB. For typical 32-bit machines, the
   maximum is usually 2^{29} - 2 bytes, or about 512 MiB. Buffer sizes
   are also limited by the amount of memory in the system.
*** Creating and Selecting Buffers
    One reason to create a new buffer is to use it for making
    temporary notes. If you try to save it, Emacs asks for the file
    name to use, and the buffer's major mode is re-established taking
    that file name into account.

    Emacs uses buffer names that start with a space for internal
    purposes. It treats these buffers specially in minor ways - for
    example, by default they do not record undo information. It is
    best to avoid using such buffer names yourself.
*** Listing Existing Buffers
    There are different indicators in the buffer list that mean
    different things:
...
*** Miscellaneous Buffer Operations
    ...
    delete its text are not allowed. (However, other commands, like
    ~C-x <RET> f~, can still mark it as modified). The mode line
    indicates read-only buffers with '~%%~' or '~%*~' near the left
    margin. Read-only buffers are usually made by subsystems such as
    Dired and Rmail that have special commands to operate on the
    text. Visiting a file whose access control says you cannot write
    it also makes the buffer read-only.

    ~rename-uniquely~ renames the current buffer to a similar name
    with a numeric suffix added to make it both different and
    unique. This command does not need an argument. It is useful for
    creating multiple shell buffers: if you rename the ~*shell*~
    buffer, then do ~M-x shell~, it makes a new shell buffer named
    ~*shell*~; meanwhile, the old shell buffer continues to exist
    under its new name. This method is also good for mail buffers,
    compilation buffers, and most Emacs features that create special
    buffers with particular names. (With some of these features, such
    as ~M-x compile~, ~M-x grep~, you need to switch to some other
    buffer before using the command again, otherwise it will reuse the
    current buffer despite the name change.)
*** Killing Buffers
    On most operating systems, killing a buffer releases the memory
    Emacs used for the buffer back to the operating system so that
    other programs can use it.
*** Operating on Several Buffers
    The Buffer Menu opened by ~C-x C-b~ does not merely list
    buffers. It also allows you to perform various operations on
    buffers, through an interface similar to Dired. You can save
    buffers, kill them (here called deleting them, for consistency
...
*** Indirect Buffers
    ...
    different markers, different major modes, and different local
    variables.

    An indirect buffer cannot visit a file, but its base buffer
    can. If you try to save the indirect buffer, that actually works
    by saving the base buffer. Killing the base buffer effectively
    kills the indirect buffer, but killing an indirect buffer has no
    effect on its base buffer.

    One way to use indirect buffers is to display multiple views of an
    outline.
*** Convenience Features and Customization of Buffer Handling
**** Making Buffer Names Unique
     When several buffers visit identically-named files, Emacs must
     ...
     example, if you visit files ~/foo/bar/mumble/name~ and
     ~/baz/quux/mumble/name~ at the same time, their buffers will be
     named '~name<bar/mumble>~' and '~name<quux/mumble>~',
     respectively. Emacs adds as many directory parts as are needed to
     make a unique name.
...
*** Concepts of Emacs Windows
    ...
    buffer may appear in more than one window; if it does, any changes
    in its text are displayed in all the windows where it appears. But
    these windows can show different parts of the buffer, because each
    window has its own value of point.

    On graphical displays, the point is indicated by a solid blinking
    cursor in the selected window, and by a hollow box in non-selected
    windows. On text terminals, the cursor is drawn only in the
    selected window.

    Commands to move point affect the value of point for the selected
    Emacs window only. They do not change the value of point in other
    Emacs windows, even those showing the same buffer. The same is
    true for buffer-switching commands such as ~C-x b~; they do not
    affect other windows at all. However there are other commands such
    as ~C-x 4 b~ that select a different window and switch buffers in
    it. Also all commands that display information in a window,
    including (for example) ~C-h f~ (~describe function~) and ~C-x
    C-b~ (~list-buffers~), usually work by displaying buffers in a
    nonselected window without affecting the selected window.

    When multiple windows show the same buffer, they can have
    different regions, because they can have different values of
    point. However, they all have the same value for the mark, because
    each buffer has only one mark position.

    Each window has its own mode line, which displays the buffer name,
    modification status and major and minor modes of the buffer that
    is displayed in the window. The selecected window's mode line
    appears in a different color.
*** Splitting Windows
*** Using Other Windows
*** Displaying in Another Window
*** Deleting and Resizing Windows
    Mouse clicks on the mode line or on window dividers provide
    another way to change window heights and to split or delete windows.
*** Displaying a Buffer in a Window
    It is a common Emacs operation to display or pop up some buffer in
    response to a user command. There are several different ways in
    which commands do this.

    Many commands, like ~C-x C-f~ (~find-file~), by default display
    the buffer by "taking over" the selected window, expecting that
    the user's attention will be diverted to that buffer.
    ...
    the various help commands, work by calling ~display-buffer~
    internally.

    Other commands do the same as ~display-buffer~, and additionally
    select the displaying window so that you can begin editing its
    buffer. The command ~M-g M-n~ (~next-error~) is one example. Such
    commands work by calling the function ~pop-to-buffer~ internally.
    ...
    ~display-buffer~, except that they:
    1. Never display in the selected window and
    2. Prefer to either create a new frame or use a window on some
       other frame to display the desired buffer.
...
**** How ~display-buffer~ works
     ...
     internally) chooses a window to display by following the steps
     given below.
     1. If the buffer should be displayed in the selected window
        regardless of other considerations, reuse the selected
        window. By default, this step is skipped, but you can tell
        Emacs not to skip it by adding a regular expression matching
        the buffer's name together with a reference to the
        ~display-buffer-same-window~ action function to the option
        ~display-buffer-alist~.
     2. Otherwise, if the buffer is already displayed in an existing
        window, reuse that window. Normally, only windows on the
        selected frame are considered, but windows on other frames are
        also reusable if you use the corresponding ~reusable-frames~
        action alist entry. See next step for an example of how to do
        that.
     3. Otherwise, optionally create a new frame and display the
        buffer there. By default, this step is skipped. To enable it,
        change the value of the option ~display-buffer-base-action~.
     4. Otherwise, try to create a new window by splitting a window on
        the selected frame, and display the buffer in that window.

	The split can be either vertical or horizontal, depending on
        the variables ~split-height-threshold~ and
        ~split-width-threshold~. These variables should have integer
        values. If ~split-height-threshold~ is smaller than the chosen
        window's height, the split puts the new window
        below. Otherwise if ~split-width-threshold~ is smaller than
        the window's width, the split puts the new window on the
        right. If neigher condition holds, Emacs tries to split so
        that the new window is below - but only if the window was not
        split before (to avoid excessive splitting).
     5. Otherwise, display the buffer in a window previously showing
        it. Normally, only windows on the selected frame are
        considered, but with a suitable ~reusable-frames~ action alist
        entry, the window may also be on another frame.
     6. Otherwise, display the buffer in an existing window on the
        selected frame.
     7. If all the above methods fail for whatever reason, create a
        new frame and display the buffer there.
**** Displaying non-editable buffers
     Some buffers are shown in windows for perusal rather than for
     editing. Help commands typically use a buffer called ~*Help*~ for
     that purpose, minibuffer completion uses a buffer called
     ~*Completions*~, etc. such buffers are usually displayed only for
     a short period of time.

     Normally, Emacs chooses the window for such temporary displays
     via ~display-buffer~, as described in the previous section. The
     ~*Completions*~ buffer, on the other hand, is normally displayed
     in a window at the bottom of the selected frame, regardless of
     the number of windows already shown on that frame.
*** Convenience Features for Window Handling
*** Window Tab Line
    Note that the Tab Line is different from the Tab Bar. Whereas tabs
    on the Tab Bar at the top of each frame are used to switch between
    window configurations containing several windows with buffers,
    tabs on the Tab Line at the top of each window are used to switch
    between buffers in the window.
** Frames and Graphical Displays
   You can create more than one frame. All frames created in the same
   Emacs session have access to the same underlying buffers and other
   data. For instance, if a buffer is being shown in more than one
   frame, any changes made to it in one frame show up immediately in
   other frames too.

   Skipping the rest of this section because honestly I don't care
   about commands that affect the frame all that much.
** International Character Set Support
   Also Skipping this because I still don't really give a damn
** Major and Minor Modes
   Important section although shorter than I assumed because many of
   the major and minor modes were already described in previous
   sections.
** Indentation
   This seems like another important section but just going to skip
   again for now. This seems fine to rely on the Emacs manual, it's
   labeled as an advaned section.
** Commands for Human Languages
   Also seems like a pretty important section. Honestly I should
   probably review this one at some point. It looks like this section
   has some important concepts
** Editing Programs
   More opportunities for fun commands and such
** Compiling and Testing Programs
   This seems extremely involved in a way I'm not sure I want to dig
   into.
** Maintaining Large Programs
   Again, Emacs might not be the tool for this as I keep looking at
   it.
** Abbrevs
   This seems extremely helpful to review at some point
** Dired, the Directory Editor
   This is honestly the section I really want to get to, but as I'm
   reviewing I think I am ready to just start tackling Org mode if I
...
*** Collision
    If you begin to modify the buffer while the visited file is locked
    by someone else, this constitutes a /collision/.
*** Shadow Copy
    You can arrange to keep identical /shadow copies/ of certain files
    in more than one place - possibly on different machines.
*** Shadow File Group
    You can arrange to keep identical shadow copies of certaing files
    in more than one place - possibly on different machines. To do
    this, first you must set up a /shadow file group/, which is a set
    of identically-named files shared between a list of sites.
*** Shadow Cluster
    A /shadow cluster/ is a group of hosts that share directories, so
    that copying to or from one of them is sufficient to update the
    file on all of them.
*** File Notifications
    By default, Auto Revert mode works using /file notifications/,
    whereby changes in the filesystem are reported to Emacs by the
    OS.
*** Auto-Saving
    From time to time, Emacs automatically saves each visited file in
    a separate file, without altering the file you actually use. This
    is called /auto-saving/.
*** Auto-Save File
    Auto-saving does not normally save in the files that you visited,
    because it can be very undesirable to save a change that you did
    not want to make permanent. Instead, auto-saving is done in a
    different file called the /auto-save file/.
*** Truename
    The /truename/ of a file is made by replacing all symbolic links
    with their target names.
*** Directory
    The file system groups files into /directories/.
*** Directory Listing
    A /directory listing/ is a list of all the files in a directory.
*** Patch
    The command ~diff~ prompts for two file names, using the
    minibuffer, and displays the differences between the two files in
    a buffer named ~*diff*~. This kind of output is called a /patch/,
    because it can be passed to the ~patch~ shell command to
    automatically apply the specified changes.
*** Hunks
    The changes specified in a patch are grouped into /hunks/, which
    are contiguous chunks of text that contain one or more changed
    lines. Hunks usually also include unchanged lines to provide
    context for the changes.
*** Hunk Header
    Each hunk is preceded by a /hunk header/, which specifies the old
    and new line numbers where the hunk's changes occur.
*** Refine
    For example, Diff mode /refines/ hunks as Emacs displays them,
    highlighting their changes with better granularity
*** File Mode
    /File modes/ determine whether a file can be read, written to, or
    executed, and by whom.
*** File Permissions
    /File permissions/ are another name for file modes.
*** Firewall
    Sometimes you may be unable to access files on a remote machine
    because a /firewall/ in between blocks the connection for security
    reasons.
*** Gateway
    ...
    reasons. If you can log in on a /gateway/ machine from which the
    target files *are* accessible, and whose FTP server supports
    gatewaying features, you can still use remote file names.
*** Quote File
    You can /quote/ an absolute file name to prevent special
...
*** File Name Cache
    ...
    by name, without having to remember exactly where it is located.
*** Fileset
    If you regularly edit a certain group of files, you can define
    them as a /fileset/.
** Using Multiple Buffers
*** Buffer
    ...
    /buffer/.
*** Selected Buffer
    At any time, one and only one buffer is /selected/.
*** Current Buffer
...
*** Selected Window
    ...
    /selected window/ is current.
*** Contents
    A buffer's /contents/ consist of a series of characters, each of
    which optionally carries a set of text properties that can specify
    more information about that character.
*** Buffer-Local Variables
    Aside from its textual contents, each buffer records several
    ...
    effect. These are stored in /buffer-local variables/ - variables
    that can have a different value in each buffer.
*** Emacs Integers
    A buffer's size cannot be larger than some maximum, which is
    defined by the largest buffer position representable by /Emacs
    integers/.
*** Read-Only Buffer
    ...
    or delete its text are not allowed.
*** Kill Buffer
    You may find it convenient to /kill/ the buffers you no longer
    need. (Some other editors call this operation /close/, and talk
    about "closing the buffer" or "closing the file" visited in the
    buffer.)
*** Buffer Menu
    The /Buffer Menu/ does not merely list buffers. It also allows you
    to perform various operations on buffers, through an interface
    similar to Dired.
*** Indirect Buffer
    An /indirect buffer/ shares the text of some other buffer. In some
    ways it is a buffer analogue of a symbolic link between files.
*** Base Buffer
    An indirect buffer shares the text of some other buffer, which is
    called the /base buffer/ of the indirect buffer.
** Multiple Windows
*** Selected Window
    At any time, one Emacs window is the /selected window/; the buffer
    this window is displaying is the current buffer.
*** Tab Line
    The /Tab Line/ shows special buttons ("tabs") for each buffer that
    was displayed in a window, and allows switching to any of these
    buffers by clicking the corresponding button.
** Frames and Graphical Displays
*** Frame
    A graphical system-level display region. We call this a /frame/,
    reserving the word "window" for the part of the frame used for
    displaying a buffer. A frame initially contains one window, but it
    can be subdivided into multiple windows. A frame normally also
...
*** Auto Revert mode
    ...
    date, you can enable Auto Revert mode. This automatically reverts
    the buffer when its visited file changes on disk.

    Auto Revert will not revert a buffer if it has unsaved changes, or
    if its file on disk is deleted or renamed.

    One use of Auto Revert mode is to "tail" a file such as a system
    log, so that changes made to that file by other programs are
    continuously displayed. To do this, just move the point to the end
    of the buffer, and it will stay there as the file contents
    change.
    ...
    relying on notifications only. However, notification is
    ineffective on certain file systems; mainly network file system on
    Unix-like machines, where files can be altered from other
    machines. For such file systems, polling may be necessary.

    In Dired buffers, Auto Revert mode refreshes the buffer when a
    file is created or deleted in the buffer's directory.
*** Global Auto Revert mode
    A buffer can get out of sync with respect to its visited file on
    disk if that file is changed by another program. To keep all file
    buffers up to date you can enable Global Auto Revert mode. This
    automatically reverts all buffers when its respective file changes
    on disk.

    Global Auto Revert mode normally only reverts file buffers.
*** Auto Revert Tail mode
    If you are sure that a file will only change by growing at the
    end, use Auto Revert Tail mode instead of Auto Revert mode. It is
    more efficient for this. Auto Revert Tail mode also works for
    remote files.
*** Auto Save mode
    Minor mode that automatically saves the file periodically in an
    altnerative file even if the user has not officially saved the
    file they are working on. This mode temporarily gets turned off if
    you delete a large block of text.
*** Auto Save Visited mode
    In this mode, auto-saving is identical to explicit saving. Note
    that this mode is orthogonal to the Auto Save mode; you can enable
    both at the same time. However, if Auto Save mode is active in
    some buffer and the obsolete ~auto-save-visited-file-name~
    variable is set to a non-nil value, that buffer won't be affected
    by this mode.
*** Diff mode
    The command ~diff~ prompts for two file names, using the
    minibuffer, and displays the differences between the two files in
    a buffer named ~*diff*~. This works by running the ~diff~ program,
    using options taken from the variable ~diff-switches~.

    The output of the ~diff~ and other similar commands is shown in
    this major mode.

    Diff mode highlights each hunk header, to distinguish it from the
    actual contents of the hunk.

    The first hunk in a patch is preceded by a file header, which
    shows the names of the new and the old versions of the file, and
    their time stamps. If a patch shows changes for more than one
    file, each file has such a header before the first hunk of that
    file's changes.

    You can edit a Diff mode buffer like any other buffer. (If it is
    read-only, you need to make it writable first.) Whenever you edit
    a hunk, Diff mode attempts to automatically correct the line
    numbers in the hunk headers, to ensure that the patch remains
    correct, and could still be applied by ~patch~.

    Diff mode arranges for hunks to be treated as compiler error
    messages by ~M-g M-n~ and other commands that handle error
    messages. Thus you can use the compilation mode commands to visit
    the corresponding source locations.
*** Auto Compression mode
    Emacs automatically uncompresses compressed files when you visit
    them, and automatically recompresses them if you alter them and
    save them. Emacs recognizes compressed files by their file
    names. File names ending in '~.gz~' indicate a file compressed
    with ~gzip~. Other endings indicate other compression programs.

    Automatic uncompression and compression apply to all the
    operations in which Emacs uses the contents of a file. This
    includes visiting it, saving it, inserting its contents into a
    buffer, loading it, and byte compiling it.
*** Tar mode
    A file whose name ends in '~.tar~' is normally an /archive/ made
    by the ~tar~ program. Emacs views these files in a special mode
    called Tar mode which provides a Dired-like list of the
    contents. You can move around through the list just as you would
    in Dired, and visit the subfiles contained in the
    archive. However, not all Dired commands are available in Tar
    mode.
...
*** Archive mode
    A separate but similar Archive mode is used for ~arc~, ~jar~,
    ~lzh~, ~zip~, ~rar~, ~7z~, and ~zoo~ archives, as well as ~exe~
    files that are self-extracting executables.

    Operations such as renaming a subfile, or changing its mode or
    owner, are supported only for some of the archive formats.

    Unlike Tar mode, Archive mode runs the archiving programs to
    unpack and repack archives. However, you don't need these programs
    to look at the archive table of contents, only to extract or
    manipulate the subfiles in the archive.
*** Tramp mode
    Allows you to find and access remote files.
*** Recentf mode
    When this mode is enabled, the '~File~' menu includes a submenu
    containing a list of recently opened files.
*** Image mode
    Mode for viewing images

    If the displayed image is wider or taller than the window in which
    it is displayed, the usual point motion keys (~C-f~, ~C-p~, and so
    forth) cause different parts of the image to be
    displayed. However, by default images are resized automatically to
    fit the window, so this is only necessary if you customize the
    default behavior by using the options ~image-auto-resize~ and
    ~image-auto-resize-on-window-resize~.
** Using Multiple Buffers
*** Midnight mode
    Midnight mode operates each day at midnight; at that time, it runs
...
*** Buffer Menu mode
    ...
    buffers. It also allows you to perform various operations on
    buffers, through an interface similar to Dired. You can save
    buffers, kill them, or display them.

    To use the Buffer Menu, type ~C-x C-b~ and switch to the window
    displaying the ~*Buffer List*~ buffer.

    The Buffer Menu is a read-only buffer, and can be changed only
    through special commands. The usual cursor motion commands can be
    used in this buffer
*** Icomplete mode
    Icomplete global minor mode provides a convenient way to quickly
    select an element among the possible completions in a
    minibuffer. When enabled, typing in the minibuffer continuously
    displays a list of possible completions that match the string you
    have typed.
*** Fido mode
    This mode is very similar to Icomplete mode, but retains some
    functionality from a popular extension called Ido mode (in fact
    the name is derived from "Fake Ido").

    Another noteworth aspect is that ~flex~ is used as the default
    completion style.
** Multiple Windows
*** Temp Buffer Resize mode
    You can make sure other temporary buffers resize to the minimum
    possible height by turning on this mode.
*** Winner mode
    Winner mode is a global minor mode that records the changes in the
    window configuration (i.e., how the frames are partitioned into
    windows), so that you can undo them.
*** Scroll All mode
    Scroll All mode is a global minor mode that causes scrolling
    commands and point motion commands to apply to every single
    window.
*** Global Tab Line mode
    The /Tab Line/ shows special buttons ("tabs") for each buffer that
    was displayed in a window, and allows switching to any of these
    buffers by clicking the corresponding button. Clicking on the ~+~
    icon adds a new buffer to the window-local tab line of buffers,
    and clicking on the ~x~ icon of a tab deletes it. The mouse wheel
    on the tab line scrolls the tabs horizontally.
* Contexts
** anywhere
   literally anywhere
** editing
...
** help-link
   Sometimes in a help-mode page there will be a cross reference to
   another help-mode page. There is at least one command that only
   works when the point is currently on one of these cross
   references.
** help-button
   Sometimes a help-mode page will have a button that doesn't lead
   anywhere but will perform an action in emacs. There is at least one
   command that only works when the point is currently on one of these
   buttons
** info
...
** collision
   If you begin to modify the buffer while the visited file is locked
   by someone else, this constitutes a collision. When Emacs detects a
   collision, it asks you what to do. This is the context relevant
   here.
** modification-date-collision
   if Emacs lacks the system permissions or cannot create lock files
   for some other reason. In these cases, Emacs can still detect the
   collision when you try to save a file, by checking the file's
   last-modification date. If the file has changed since the last time
   Emacs visited or saved it, that implies that changes have been made
   in some other way, and will be lost if Emacs proceeds with
   saving. Emacs then displays a warning message and asks for
   confirmation before saving.
** recover-session
   After running the command ~recover-session~ a mode pops up with
   different interrupted sessions of Emacs on different lines. There
   are some commands that work in this specific context
** diff
   Diff mode
** tar
   Tar mode
...
** image
   Image mode
** buffer-menu
   Any time a Buffer Menu is the currnet buffer.
** icomplete
   This is when you are in the minibuffer when Incomplete mode is
   enabled. Fido mode also has access to all of the commands of
   Icomplete.
** fido
   Commands that are only available while in the minibuffer that can
   have completions and when Fido mode is enabled. These are Fido
   specific commands not commands related to both Icomplete and Fido
   mode.
** mode-line
   Sometimes mouse commands can have special things happen when
   you click the mouse on the mode line.
** multiple-windows
   This is when multiple windows are visible on a single frame.
** prog
   Some modes are contextualized as "prog-modes" they are modes where
   you are usually writing in a programming language.
** winner
   Winner mode which helps to undo changes in window configuration
* Commands
  :PROPERTIES:
  :COLUMNS:  %20ITEM(Command) %20Command(Key) %20Context %20Argument %20TAGS %20Description
  :ID:       commands
  :END:
** The Organization of the Screen
...
*** goto-line                                                    :nav:buffer:
    ...
    argument. With a plain prefix argument, reads a number /n/ using
    the minibuffer, selects the most recently selected buffer other
    than the current buffer in another window, and then moves point to
    the beginning of line number /n/ in that buffer. This is mainly
    useful in a buffer that refers to line numbers in another buffer:
    if point is on or just after a number, this command uses that
    number as the default for /n/.
...
*** CO Steal                                                          :files:
    :PROPERTIES:
    :Command:  s
    :Description: Steal the lock
    :Context:  collision
    :END:
*** CO Proceed                                                        :files:
    :PROPERTIES:
    :Command:  p
    :Description: Go ahead and edit file despite being locked
    :Context:  collision
    :END:
*** CO Quit                                                      :files:undo:
    :PROPERTIES:
    :Command:  q
    :Description: Quit and buffer contents remain unchanged
    :Context:  collision
    :END:
*** MDC Yes                                                           :files:
    :PROPERTIES:
    :Command:  yes
    :Description: Save file despite potential collision
    :Context:  modification-date-collision
    :END:
*** MDC No                                                       :files:undo:
    :PROPERTIES:
    :Command:  no OR C-g
    ...
    :Context:  modification-date-collision
    :END:
*** diff-buffer-with-file                              :files:inform:display:
    :PROPERTIES:
    :Description: Compare buffer to its file
    :Context:  editing
    :END:
*** shadow-copy-files                                                 :files:
    :PROPERTIES:
    :Description: Copy all pending shadow files
    :Context:  navigating
    :END:
*** shadow-define-cluster                                             :files:
    :PROPERTIES:
    :Description: Define a shadow file cluster
    :Context:  navigating
    :END:
*** shadow-initialize                                                 :files:
    :PROPERTIES:
    :Description: Set up file shadowing
    :Context:  navigating
    :END:
*** shadow-define-literal-group                                       :files:
    :PROPERTIES:
    :Description: Declare a single file to be shared between sites
    :Context:  navigating
    :END:
*** shadow-define-regexp-group                                        :files:
    :PROPERTIES:
    :Description: Make all files that match regexp be shared between hosts
    :Context:  navigating
    :END:
*** shadow-cancel                                                     :files:
    :PROPERTIES:
    :Description: Cancel instruction to shadow some files
    :Context:  navigating
    :END:
*** time-stamp                                                   :files:line:
    :PROPERTIES:
    :Description: Update time stamp in file manually
    :Context:  editing
    :END:
*** revert-buffer                                                :files:undo:
    :PROPERTIES:
    :Description: Go back to saved version of file in buffer
    :Context:  editing
    :END:
    Since reverting unintentionally could lose a lot of work, Emacs
    asks for confirmation first.

    This command tries to position point in such a way that, if the
    file was edited only slightly, you will be at approximately the
    same part of the text as before. But if you have made major
    changes, point may end up in a totally different location.
*** auto-revert-mode                                             :files:mode:
    :PROPERTIES:
    :Description: Toggle Auto Revert mode
    :Context:  editing
    :END:
*** global-auto-revert-mode                                      :files:mode:
    :PROPERTIES:
    :Description: Toggle Global Auto Revert mode
    :Context:  editing
    :END:
*** auto-revert-tail-mode                                        :files:mode:
    :PROPERTIES:
    :Description: Toggle Auto Revert Tail mode
    :Context:  editing
    :END:
*** auto-save-mode                                               :files:mode:
    :PROPERTIES:
    :Description: Toggle Auto Save mode
    :Context:  editing
    :END:
*** auto-save-visited-mode                                       :files:mode:
    :PROPERTIES:
    :Description: Toggle Auto Save Visited mode
    :Context:  editing
    :END:
*** do-auto-save                                                      :files:
    :PROPERTIES:
    :Description: Perform an auto-save explicitly
    :Context:  editing
    :END:
*** recover-file                                                      :files:
    :PROPERTIES:
    :Description: Recover from loss of data with auto-save file
    :Context:  navigating
    :END:
    If the auto-save file is older this command does not offer to read
    it.
*** recover-session                                                   :files:
    :PROPERTIES:
    :Description: Recover all files you were editing last session
    :Context:  navigating
    :END:
*** recover-session-finish                                            :files:
    :PROPERTIES:
    :Command:  C-c C-c
    :Description: Choose one saved session to recover auto-save files from
    :Context:  recover-session
    :END:
*** dired                                                    :files:nav:edit:
    :PROPERTIES:
    :Command:  C-x d
    :Description: Turn on directory browser feature "Dired"
    :Context:  navigating
    :END:
*** list-directory                                             :files:inform:
    :PROPERTIES:
    :Command:  C-x C-d
    :Description: Display a brief directory listing
    :Context:  navigating
    :Argument: plain
    :END:
    With a plain argument, display a verbose directory listing

    It reads using the minibuffer a file name which is either a
    directory to be listed or a wildcard-containing pattern for the
    files to be listed.
*** make-directory                                               :files:edit:
    :PROPERTIES:
    :Description: Create a new directory with given name
    :Context:  navigating
    :END:
*** delete-directory                                      :files:edit:delete:
    :PROPERTIES:
    :Description: Delete given directory by name
    :Context:  navigating
    :END:
    If directory isn't empty, you will be asked whether you want to
    delete it recursively.
*** diff                                                :files:inform:buffer:
    :PROPERTIES:
    :Description: Displays differences between two given files
    :Context:  navigating
    :END:
*** ediff                                               :files:inform:buffer:
    :PROPERTIES:
    :Description: A much more sophisticated alternative to ~diff~
    :Context:  navigating
    :END:
    [[https://www.gnu.org/software/emacs/manual/html_node/ediff/index.html#Top][manual]]
*** diff-backup                                         :files:inform:buffer:
    :PROPERTIES:
    :Description: Compares given file with its most recent backup
    :Context:  navigating
    :END:
    If you specify the name of a backup file, this compares it with
    the source file that it is a backup of. In all other respects,
    this behaves like ~diff~.
*** diff-buffer-with-file                               :files:inform:buffer:
    :PROPERTIES:
    :Description: Compares given buffer with its corresponding file
    :Context:  navigating
    :END:
*** diff-buffers                                              :inform:buffer:
    :PROPERTIES:
    :Description: Compares contents of two given buffers
    :Context:  navigating
    :END:
*** compare-windows                                    :window:buffer:inform:
    :PROPERTIES:
    :Description: Compares text in curr window with prev window
    :Context:  editing
    :Argument: numeric
    :END:
    Comparison starts at point in each window, after pushing each
    initial point value on the mark ring in its respective
    buffer. Then it moves point forward in each window, one character
    at a time, until it reaches characters that don't match. Then the
    command exits.

    If point in the two windows is followed by non-matching text when
    this command starts, the command tries heuristically to advance up
    to matching text in the two windows, and then exits. So if you use
    this command repeatedly, each time it either skips one matching
    range or finds the start of another.

    With a numeric argument, this command ignores changes in
    whitespace.
*** diff-mode                                            :buffer:inform:mode:
    :PROPERTIES:
    :Description: Toggle Diff mode manually
    :Context:  editing
    :END:
*** next-error                                                          :nav:
    :PROPERTIES:
...
*** diff-hunk-next                                                      :nav:
    :PROPERTIES:
    :Command:  M-n
    :Description: Move to next hunk-start
    :Context:  diff
    :Argument: numeric
    :END:
    With prefix argument /n/, move forward to the /n/-th next hunk.

    By default, Diff mode refines hunks as Emacs displays them,
    highlighting their changes with better granularity. Check
    ~diff-refine~ for some details.
*** diff-hunk-prev                                                      :nav:
    :PROPERTIES:
    :Command:  M-p
    :Description: Move to previous hunk-start
    :Context:  diff
    :Argument: numeric
    :END:
    With prefix argument /n/, move back to the /n/-th previous
    hunk. Like ~M-n~. Check ~diff-refine~ for some details.
*** diff-file-prev                                                      :nav:
    :PROPERTIES:
    :Command:  M-{
    :Description: Move to previous file-start in a multi-file patch
    :Context:  diff
    :Argument: numeric
    :END:
    With prefix argument /n/, move forward to the start of the /n/-th
    next file.
*** diff-file-next                                                      :nav:
    :PROPERTIES:
    :Command:  M-}
    :Description: Move to next file-start, in a multi-file patch
    :Context:  diff
    :Argument: numeric
    :END:
    With prefix argument /n/, move back to the start of the /n/-th
    previous file.
*** diff-hunk-kill                                                     :kill:
    :PROPERTIES:
    :Command:  M-k
    :Description: Kill hunk at point
    :Context:  diff
    :END:
*** diff-file-kill                                                     :kill:
    :PROPERTIES:
    :Command:  M-K
    :Description: In a multi-file patch, kill current file part
    :Context:  diff
    :END:
*** diff-apply-hunk                                                    :edit:
    :PROPERTIES:
    :Command:  C-c C-a
    :Description: Apply this hunk to its target (old) file
    :Context:  diff
    :Argument: plain
    :END:
    With a plain prefix argument, revert this hunk, i.e. apply the
    reverse of the hunk, which changes the "new" version into the
    "old" version.
*** diff-refine-hunk                                                :display:
    :PROPERTIES:
    :Command:  C-c C-b
    :Description: Highlight changes of hunk at point with granularity
    :Context:  diff
    :END:
*** diff-goto-source                                                    :nav:
    :PROPERTIES:
    :Command:  C-c C-c
    :Description: Go to source file and line corresponding to this hunk
    :Context:  diff
    :Argument: plain, double
    :END:
    By default, this jumps to the "new" version of the file, the one
    shown first on the header. With a prefix argument, jump to the
    "old" version instead. If ~diff-jump-to-old-file~ is non-nil, this
    command by default jumps to the "old" file, and the meaning of the
    prefix argument is reversed. With a double argument, then this
    command also sets ~diff-jump-to-old-file~ to non-nil for the next
    invocation. If the source file is under version control, this
    jumps to the work file by default. With a prefix argument, jump to
    the "old" revision of the file, when point is on the old line, or
    otherwise jump to the "new" revision.
*** diff-ediff-patch                                       :edit:mode:inform:
    :PROPERTIES:
    :Command:  C-c C-e
    :Description: Start an Ediff session with patch
    :Context:  diff
    :END:
*** diff-restrict-view                                              :display:
    :PROPERTIES:
    :Command:  C-c C-n
    :Description: Restrict view to the current hunk
    :Context:  diff
    :Argument: plain
    :END:
    With a prefix argument, restrict the view to the current file of a
    multiple-file patch. To widen again, use ~C-x n w~ (~widen~).
*** diff-reverse-direction                              :display:mode:inform:
    :PROPERTIES:
    :Command:  C-c C-r
    :Description: Reverse direction of comparison for entire buffer
    :Context:  diff
    :END:
    With a prefix argument, reverse the direction only inside the
    current region. Reversing the direction means changing the hunks
    and the file-start headers to produce a patch that would change
    the "new" version into the "old" one.
*** diff-split-hunk                                                  :inform:
    :PROPERTIES:
    :Command:  C-c C-s
    :Description: Split hunk at point into two separate hunks
    :Context:  diff
    :END:
    This inserts a hunk header and modifies the header of the current
    hunk. This command is useful for manually editing patches, and
    only works with the unified diff format produced by the ~-u~ or
    ~--unified~ options to the ~diff~ program. If you need to split a
    hunk in the context diff format produced by the ~-c~ or
    ~--context~ options to ~diff~, first convert the buffer to the
    unified diff format with ~C-c C-u~ (~diff-context->unified~).
*** diff-unified->context                                       :mode:inform:
    :PROPERTIES:
    :Command:  C-c C-d
    :Description: Convert entire buffer to context diff format
    :Context:  diff
    :Argument: plain
    :END:
    With a prefix argument, convert only the hunks within the region.
*** diff-context->unified                                       :mode:inform:
    :PROPERTIES:
    :Command:  C-c C-u
    :Description: Convert entire buffer to unified diff format
    :Context:  diff
    :Argument: plain
    :END:
    With a prefix argument, convert unified format to context
    format. When the mark is active, convert only the hunks within the
    region from context diff format to unified diff format.
*** diff-ignore-whitespace-hunk                                      :inform:
    :PROPERTIES:
    :Command:  C-c C-w
    :Description: Re-generate current hunk, disregarding whitespace changes
    :Context:  diff
    :END:
*** diff-add-change-log-entries-other-window                    :inform:edit:
    :PROPERTIES:
    :Command:  C-x 4 A
    :Description: Generate ChangeLog entry for each hunk
    :Context:  diff
    :END:
    This creates a skeleton of the log of changes that you can later
    fill with the actual descriptions of the changes. ~C-x 4 a~ itself
    in Diff mode operates on behalf of the current hunk's file, but
    gets the function name from the patch itself. This is useful for
    making log entries for functions that are deleted by the patch.
*** diff-delete-trailing-whitespace                             :edit:delete:
    :PROPERTIES:
    :Description: Search and remove trailing whitespace in patch and source
    :Context:  diff
    :Argument: plain
    :END:
    This command does not save the modification that it makes, so you
    can decide whether to save the changes (the list of modified files
    is displayed in the echo area). With a prefix argument it tries to
    modify the original ("old") source files rather than the patched
    ("new") source files.
*** copy-file                                                    :edit:files:
    :PROPERTIES:
    :Description: Copies contents of given old to new file
    :Context:  navigating
    :END:
*** copy-directory                                               :edit:files:
    :PROPERTIES:
    :Description: Copies directories, similar to ~cp -r~ shell command
    :Context:  navigating
    :END:
    If /new/ is a directory name, it creates a copy of the /old/
    directory and puts it in /new/. Otherwise it copies all the
    contents of /old/ into a new directory named /new/.
*** rename-file                                                  :edit:files:
    :PROPERTIES:
    :Description: Renames file old as new
    :Context:  editing
    :END:
    If the file name /new/ already exists, you must confirm with ~yes~
    or renaming is not done; this is because renaming causes the old
    meaning of the name /new/ to be lost. If /old/ and /new/ are on
    different file systems, the file /old/ is copied and deleted.

    If a file is under version control, you should rename it using
    ~vc-rename-file~ instead
*** vc-rename-file                                               :edit:files:
    :PROPERTIES:
    :Description: Renames file old as new when old is under version control
    :Context:  editing
    :END:
*** add-name-to-file                                             :edit:files:
    :PROPERTIES:
    :Description: Adds additional name to existing file
    :Context:  navigating
    :END:
    Does not remove old name. The new name is created as a hard link
    to the existing file. The new name must belong on the same file
    system that the file is on. On MS-Windows, this command works only
    if the file resides in an NTFS file system. On MS-DOS, and some
    remote system types, it works by copying the file.
*** make-symbolic-link                                           :edit:files:
    :PROPERTIES:
    :Description: Creates a symbolic link named new, which points at target
    :Context:  navigating
    :END:
    The effect is that future attempts to open file /new/ will refer
    to whatever file is named /target/ at the time the opening is
    done, or will get an error if the name /target/ is nonexistent at
    that time. This command does not expand the argument /target/, so
    that it allows you to specify a relative name as the target of the
    link. However, this command does expanding leading '~' in /target/
    so that you can easily specify home directories, and strips
    leading '~/:~' so that you can specify relative names beginning
    with literal '~' or '/:'. On MS-Windows, this command works only
    on MS Windows Vista and later. When /new/ is remote, it works
    depending on the system type.
*** delete-file                                                :files:delete:
    :PROPERTIES:
    :Description: Prompts for a file and deletes it
    :Context:  navigating
    :END:
    If you are deleting many files in one directory, it may be more
    convenient to use Dired rather than this command.

    If a file is under version control, you should delete it using
    ~vc-delete-file~ instead of this command.
*** move-file-to-trash                                         :files:delete:
    :PROPERTIES:
    :Description: Moves a file system into the system Trash or Recycle Bin
    :Context:  navigating
    :Argument: plain
    :END:
    This is a facility available on most operating systems; files that
    are moved into the Trash can be brought back later if you change
    your mind. (The way to restore trashed files is system-dependent.)

    Supplying a prefix argument to this command performs the deletion
    outright, instead of using the Trash, regardless of
    ~delete-by-moving-to-trash~.
*** vc-delete-file                                             :delete:files:
    :PROPERTIES:
    :Description: Delete file that is under version controll
    :Context:  navigating
    :END:
*** insert-file                                           :edit:files:buffer:
    :PROPERTIES:
    :Command:  C-x i
    :Description: Inserts copy of given file into current buffer at point
    :Context:  editing
    :END:
    Leaves point unchanged before the contents. The position after the
    inserted contents is added to the mark ring, without activating
    the mark.
*** insert-file-literally                                 :edit:files:buffer:
    :PROPERTIES:
    :Description: Insert copy of given file literally into current buffer
    :Context:  editing
    :END:
    Literal copy of file is treated as a sequence of ASCII character
    with no special encoding or conversion, similar to the
    ~find-file-literally~ command.
*** write-region                                          :region:edit:files:
    :PROPERTIES:
    :Description: Copies region into given file
    :Context:  navigating
    :END:
*** set-file-modes                                                    :files:
    :PROPERTIES:
    :Description: Applies to given file the given file mode
    :Context:  navigating
    :END:
    This command reads file modes using the same symbolic or octal
    format accepted by the ~chmod~ command; for instance, '~u+x~' means
    to add execution permission for the user who owns the file. It has
    no effect on operating systems that do not support file
    modes. Here is a reminder of the basic syntax:
    - ~u~ :: User
    - ~g~ :: Group
    - ~o~ :: Others
    - ~r~ :: Read
...
*** chmod                                                             :files:
    :PROPERTIES:
    :Description: Convenience alias for ~set-file-modes~
    :Context:  navigating
    :END:
*** auto-compression-mode                                        :mode:files:
    :PROPERTIES:
    :Description: Toggle Auto Compression mode
    :Context:  anywhere
    :END:
*** tar-extract                                                :buffer:files:
    :PROPERTIES:
    :Command:  e OR f OR <RET>
    :Description: Extract component file into its own buffer
    :Context:  tar
    :END:
    You can edit extracted file in new buffer, and if you save the
    buffer, the edited version will replace the version in the Tar
    buffer. Clicking with the mouse on the file name in the Tar buffer
    does likewise.
*** tar-view                                              :files:buffer:mode:
    :PROPERTIES:
    :Command:  v
    :Description: Extract a file into a buffer in View mode
    :Context:  tar
    :END:
*** tar-extract-other-window                            :files:buffer:window:
    :PROPERTIES:
    :Command:  o
    :Description: Extract file and display in another window
    :Context:  tar
    :END:
*** tar-new-entry                                                :files:edit:
    :PROPERTIES:
    :Command:  I
    :Description: Adds a new regular file to the archive
    :Context:  tar
    :END:
    The file is initially empty, but can readily be edited using the
    other tar commands. The command inserts the new file before the
    current one, so that using it on the topmost line of the Tar
    buffer makes the new file the first one in the archive, and using
    it at the end of the buffer makes it the last one.
*** tar-flag-deleted                                      :files:edit:delete:
    :PROPERTIES:
    :Command:  d
    :Description: Marks a file for deletion
    :Context:  tar
    :END:
*** tar-expunge                                                :files:delete:
    :PROPERTIES:
    :Command:  x
    :Description: Delete all files that have been marked for deletion
    :Context:  tar
    :END:
*** tar-unflag                                                   :files:edit:
    :PROPERTIES:
    :Command:  u
    :Description: Unmark file for deletion
    :Context:  tar
    :END:
*** tar-copy                                                     :files:edit:
    :PROPERTIES:
    :Command:  C
    :Description: Copies a file from archive to disk
    :Context:  tar
    :END:
*** tar-rename-entry                                             :files:edit:
    :PROPERTIES:
    :Command:  R
    :Description: Rename a file within the archive
    :Context:  tar
    :END:
*** revert-buffer                                                :files:undo:
    :PROPERTIES:
    :Command:  g
    :Description: Reverts buffer from archive on disk
    :Context:  tar
    :END:
*** tar-chmod-entry                                              :files:edit:
    :PROPERTIES:
    :Command:  M
    :Description: Change protection bits associated with entry
    :Context:  tar
    :END:
*** tar-chgrp-entry                                              :files:edit:
    :PROPERTIES:
    :Command:  G
    :Description: Change group-id associated with entry
    :Context:  tar
    :END:
*** tar-chown-entry                                              :files:edit:
    :PROPERTIES:
    :Command:  O
    :Description: Change user-id associated with this entry
    :Context:  tar
    :END:
*** archive-extract                                            :buffer:files:
    :PROPERTIES:
    :Command:  e OR f OR <RET>
    :Description: Extract component file into its own buffer
    :Context:  archive
    :END:
    You can edit extracted file in new buffer, and if you save the
    buffer, the edited version will replace the version in the Archive
    buffer. Clicking with the mouse on the file name in the Archive buffer
    does likewise.
*** archive-view                                          :files:buffer:mode:
    :PROPERTIES:
    :Command:  v
    :Description: Extract a file into a buffer in View mode
    :Context:  archive
    :END:
*** archive-extract-other-window                        :files:buffer:window:
    :PROPERTIES:
    :Command:  o
    :Description: Extract file and display in another window
    :Context:  archive
    :END:
*** archive-new-entry                                            :files:edit:
    :PROPERTIES:
    :Command:  I
    :Description: Adds a new regular file to the archive
    :Context:  archive
    :END:
    The file is initially empty, but can readily be edited using the
    other archive commands. The command inserts the new file before the
    current one, so that using it on the topmost line of the Archive
    buffer makes the new file the first one in the archive, and using
    it at the end of the buffer makes it the last one.
*** archive-flag-deleted                                  :files:edit:delete:
    :PROPERTIES:
    :Command:  d
    :Description: Marks a file for deletion
    :Context:  archive
    :END:
*** archive-expunge                                            :files:delete:
    :PROPERTIES:
    :Command:  x
    :Description: Delete all files that have been marked for deletion
    :Context:  archive
    :END:
*** archive-unflag                                               :files:edit:
    :PROPERTIES:
    :Command:  u
    :Description: Unmark file for deletion
    :Context:  archive
    :END:
*** archive-copy                                                 :files:edit:
    :PROPERTIES:
    :Command:  C
    :Description: Copies a file from archive to disk
    :Context:  archive
    :END:
*** archive-rename-entry                                         :files:edit:
    :PROPERTIES:
    :Command:  R
    :Description: Rename a file within the archive
    :Context:  archive
...
*** revert-buffer                                                :files:undo:
    ...
    :Context:  archive
    :END:
*** archive-chmod-entry                                          :files:edit:
    :PROPERTIES:
    :Command:  M
    :Description: Change protection bits associated with entry
    :Context:  archive
    :END:
*** archive-chgrp-entry                                          :files:edit:
    :PROPERTIES:
    :Command:  G
    :Description: Change group-id associated with entry
    :Context:  archive
    :END:
*** archive-chown-entry                                          :files:edit:
    :PROPERTIES:
    :Command:  O
    :Description: Change user-id associated with this entry
    :Context:  archive
    :END:
*** archive-mark                                                 :files:edit:
    :PROPERTIES:
    :Command:  m
    :Description: mark this member for group operations
    :Context:  archive
    :END:
*** archive-unmark-all-files                                     :files:edit:
    :PROPERTIES:
    :Command:  M-<DEL>
    :Description: Remove all marks
    :Context:  archive
    :END:
*** archive-alternate-display                                 :files:display:
    :PROPERTIES:
    :Command:  a
    :Description: Toggle alternative display
    :Context:  archive
    :END:
*** file-cache-minibuffer-complete                      :files:complete:echo:
    :PROPERTIES:
    :Command:  C-<TAB>
    :Description: Completes minibuffer file name using the file name cache
    :Context:  minibuffer
    :END:
    If you repeat ~C-<TAB>~, that cycles through the possible
    completions of what you had originally typed. (However, note that
    the ~C-<TAB>~ character cannot be typed on most text terminals.)
*** file-cache-add-directory                                 :files:complete:
    :PROPERTIES:
    :Description: Add each file name in given directory to file name cache
    :Context:  navigating
    :END:
*** file-cache-add-directory-using-find                      :files:complete:
    :PROPERTIES:
    :Description: Add all files and nested files from directory to cache
    :Context:  navigating
    :END:
*** file-cache-add-directory-using-locate                    :files:complete:
    :PROPERTIES:
    :Description: Add each file and nexted file using ~locate~ to cache
    :Context:  navigating
    :END:
*** file-cache-add-directory-list                            :files:complete:
    :PROPERTIES:
    :Description: Add each file in each directory from variable to cache
    :Context:  navigating
    :END:
*** file-cache-clear-cache                                            :files:
    :PROPERTIES:
    :Description: Clear the cache
    :Context:  navigating
    :END:
*** file-cache-display                                 :files:display:inform:
    :PROPERTIES:
    :Description: View contents of file name cache
    :Context:  navigating
    :END:
*** recentf-mode                                                      :files:
    :PROPERTIES:
    :Description: Toggle Recentf mode
    :Context:  navigating
    :END:
*** recentf-save-list                                                 :files:
    :PROPERTIES:
    :Description: Saves current ~recentf-list~ to a file
    :Context:  anywhere
    :END:
*** recentf-edit-list                                            :files:edit:
    :PROPERTIES:
    :Description: Edit ~recentf-list~
    :Context:  anywhere
    :END:
*** ffap                                                              :files:
    :PROPERTIES:
    :Description: Generalizes ~find-file~ with better heuristic defaults
    :Context:  navigating
    :END:
*** image-toggle-display                               :files:display:coding:
    :PROPERTIES:
    :Command:  C-c C-c
    :Description: Toggle between displaying image and raw byte representation
    :Context:  image
    :END:
*** image-toggle-hex-display                           :files:display:coding:
    :PROPERTIES:
    :Command:  C-c C-x
    :Description: Toggle between displaying image vs hex representation
    :Context:  image
    :END:
*** image-transform-fit-both                                  :files:display:
    :PROPERTIES:
    :Command:  s b
    :Description: Fit image to both window height and width
    :Context:  image
    :END:
*** image-transform-set-scale                                 :files:display:
    :PROPERTIES:
    :Command:  s s
    :Description: Scale image specifying a scale factor
    :Context:  image
    :END:
*** image-transform-reset                                     :files:display:
    :PROPERTIES:
    :Command:  s 0
    :Description: Reset all transformations to initial state
    :Context:  image
    :END:
*** image-next-file                                               :files:nav:
    :PROPERTIES:
    :Command:  N
    :Description: Visit next image file in same directory
    :Context:  image
    :END:
*** image-previous-file                                           :files:nav:
    :PROPERTIES:
    :Command:  p
    :Description: Visit previous image file in the same directory
    :Context:  image
    :END:
*** image-mode-mark-file                                              :files:
    :PROPERTIES:
    :Command:  m
    :Description: Mark current file in relevant Dired buffer
    :Context:  image
    :END:
    If no such buffer is open, the directory is opened in a new
    buffer.
*** image-mode-unmark-file                                            :files:
    :PROPERTIES:
    :Command:  u
    :Description: Unmark file in Dired buffer
    :Context:  image
    :END:
*** image-mode-copy-file-name-as-kill                            :files:kill:
    :PROPERTIES:
    :Command:  w
    :Description: Copy current buffers file name to kill ring
    :Context:  image
    :END:
*** image-toggle-animation                                    :files:display:
    :PROPERTIES:
    :Command:  <RET>
    :Description: Starts or stops animation if relevant
    :Context:  image
    :END:
*** image-next-frame                                      :files:display:nav:
    :PROPERTIES:
    :Command:  f
    :Description: Step through to next individual frame of animation
    :Context:  image
    :Argument: numeric
    :END:
    With a numeric argument step through several frames at once.
*** image-previous-frame                                  :files:display:nav:
    :PROPERTIES:
    :Command:  b
    :Description: Step back to the previous individual frame in animation
    :Context:  image
    :Argument: numeric
    :END:
    With a numeric argument step through several frames at once.
*** image-goto-frame                                      :files:display:nav:
    :PROPERTIES:
    :Command:  F
    :Description: Go to a specific frame indext from 1
    :Context:  image
    :END:
*** image-increase-speed                                      :files:display:
    :PROPERTIES:
    :Command:  a +
    :Description: Increase the speed of the animation
    :Context:  image
    :END:
*** image-decrease-speed                                      :files:display:
    :PROPERTIES:
    :Command:  a -
    :Description: Decrease the speed of the animation
    :Context:  image
    :END:
*** image-reverse-speed                                       :files:display:
    :PROPERTIES:
    :Command:  a r
    :Description: Reverse the animation with same current speed
    :Context:  image
    :END:
*** image-reset-speed                                         :files:display:
    :PROPERTIES:
    :Command:  a 0
    :Description: Reset the speed to the original value
    :Context:  image
    :END:
*** filesets-add-buffer                                   :files:buffer:edit:
    :PROPERTIES:
    :Description: Add current file to given fileset name
    :Context:  navigating
    :END:
    If there is no fileset name, this creates a new one, which
    initially contains only the current file.
*** filesets-remove-buffer                                :files:buffer:edit:
    :PROPERTIES:
    :Description: Remove current file from given fileset name
    :Context:  navigating
    :END:
*** filesets-edit                                                :files:edit:
    :PROPERTIES:
    :Description: Edit fileset directly in a customize buffer
    :Context:  anywhere
    :END:
    Normally, a fileset is a simple list of files, but you can also
    define a fileset as a regular expression matching file names. Some
    examples of these more complicated filesets are shown in the
    Customize buffer. Remember to select '~Save for future sessions~'
    if you want to use the same filesets in the future Emacs sessions.
*** filesets-open                                      :files:display:buffer:
    :PROPERTIES:
    :Description: Visit all the files in a fileset
    :Context:  navigating
    :END:
*** filesets-close                                               :files:undo:
    :PROPERTIES:
    :Description: Close all visited files in a fileset
    :Context:  navigating
    :END:
*** filesets-run-cmd                                     :files:edit:command:
    :PROPERTIES:
    :Description: Run a shell command on all the files in a fileset
    :Context:  navigating
    :END:
** Using Multiple Buffers
*** switch-to-buffer                                                 :buffer:
    ...
    :Context:  navigating
    :END:
    If you specify a buffer that does not exist, this command creates
    a new, empty buffer that is not visiting any file, and selects it
    for editing. The default value is Fundamental mode. See the
    variable ~major-mode~.
*** switch-to-buffer-other-window                             :buffer:window:
    :PROPERTIES:
    :Command:  C-x 4 b
    :Description: Select given buffer in another window
    :Context:  navigating
    :END:
*** switch-to-buffer-other-frame                               :buffer:frame:
    :PROPERTIES:
    :Command:  C-x 5 b
    :Description: Select given buffer in a separate frame
    :Context:  navigating
    :END:
*** previous-buffer                                              :buffer:nav:
    :PROPERTIES:
    :Command:  C-x <LEFT>
    :Description: Select the previous buffer in buffer list
    :Context:  navigating
    :END:
*** next-buffer                                                  :buffer:nav:
    :PROPERTIES:
    :Command:  C-x <RIGHT>
    :Description: Select next buffer in buffer list
    :Context:  navigating
    :END:
*** list-buffers                               :buffer:display:inform:window:
    :PROPERTIES:
    :Command:  C-x C-b
    ...
    buffers that were current most recnetly come first.

    With a plain argument, you can list only buffers that are visiting
    files.

    This command omits buffers whose names begin with a space, unless
    they visit files: such buffers are used internally by Emacs.
*** read-only-mode                                                   :buffer:
    :PROPERTIES:
    :Command:  C-x C-q
    :Description: Toggle read-only status of buffer
    :Context:  navigating
    :END:
*** rename-buffer                                               :buffer:edit:
    :PROPERTIES:
    :Description: Change name of the current buffer
    :Context:  navigating
    :END:
*** rename-uniquely                                             :buffer:edit:
    :PROPERTIES:
    :Description: Rename current buffer by adding '~<number>~' to end
    :Context:  navigating
    :END:
*** view-buffer                                                      :buffer:
    :PROPERTIES:
    :Description: Scroll through given buffer in View mode
    :Context:  navigating
    :END:
*** set-buffer-file-coding-system                        :buffer:coding:edit:
    :PROPERTIES:
    :Command:  C-x RET f
    :Description: Set file coding-system of current buffer to given system
    :Context:  navigating
    :END:
*** shell                                                    :command:buffer:
    :PROPERTIES:
    :Description: Run an inferior shell
    :Context:  anywhere
    :END:
*** kill-buffer                                                 :buffer:kill:
    ...
    :Context:  navigating
    :END:
    The default, used if you type just ~<RET>~ in the minibuffer, is
    to kill the current minibuffer. If you kill the current buffer,
    another buffer becomes current: one that was current in the recent
    past but is not displayed in any window now. If you ask to kill a
    file-visiting buffer that is modified, then you must confirm with
    ~yes~ before the buffer is killed.
*** kill-some-buffers                                      :buffer:kill:echo:
    :PROPERTIES:
    :Description: Offer to kill each buffer, one by one
    :Context:  navigating
    :END:
*** kill-matching-buffers                                       :buffer:kill:
    :PROPERTIES:
    :Description: Offer to kill all buffers matching a regular expression
    :Context:  navigating
    :Argument: plain
    :END:
    With a plain argument, this can kill internal buffers as well
*** clean-buffer-list                                           :buffer:kill:
    :PROPERTIES:
    :Description: Kills all unmodified and unvisited buffers
    :Context:  anywhere
    :END:
    An ordinary buffer is killed if it has not been displayed for
    three days; however, you can specify certain buffers that should
    never be killed automatically, and others that should be killed if
    they have been unused for a mere hour. These defaults, and other
    aspects of this command's behavior, can be controlled by
    customizing several options described in the doc string of
    ~clean-buffer-list~.
*** buffer-menu                                                      :buffer:
    :PROPERTIES:
    :Description: Begin editing a buffer listing all Emacs buffers
    :Context:  anywhere
    :END:
*** buffer-menu-other-window                                  :buffer:window:
    :PROPERTIES:
    :Description: Begin editing a buffer listing in another window
    :Context:  anywhere
    :END:
*** Buffer-menu-delete                                    :buffer:delete:nav:
    :PROPERTIES:
    :Command:  d
    :Description: Flag buffer for deletion, then move point next line
    :Context:  buffer-menu
    :Argument: numeric
    :END:
    The deletion flag is indicated by the character '~D~' on the line,
    before the buffer name. The deletion occurs only when you type the
    ~Buffer-menu-execute~ command.

    With a numeric argument /n/ marks next /n/ lines for deletion
*** Buffer-menu-delete-backwards                          :buffer:delete:nav:
    :PROPERTIES:
    :Command:  C-d
    :Description: Flag buffer for deletion, then move point previous line
    :Context:  buffer-menu
    :Argument: numeric
    :END:
    See ~Buffer-menu-delete~ for more details

    With a numeric argument /n/ marks previous /n/ lines for deletion
*** Buffer-menu-save                                      :buffer:files:edit:
    :PROPERTIES:
    :Command:  s
    :Description: Flag buffer for saving
    :Context:  buffer-menu
    :END:
    The save flag is indicated by the character '~S~' on the line,
    before the buffer name. The saving occurs only when you use
    ~Buffer-menu-execute~ command.
*** Buffer-menu-execute                                         :buffer:edit:
    :PROPERTIES:
    :Command:  x
    :Description: Perform all flagged deletions and saves
    :Context:  buffer-menu
    :END:
*** Buffer-menu-unmark                                               :buffer:
    :PROPERTIES:
    :Command:  u
    :Description: Remove all flags from current line, move point down
    :Context:  buffer-menu
    :END:
*** Buffer-menu-backup-unmark                                        :buffer:
    :PROPERTIES:
    :Command:  <DEL>
    :Description: Move to previous line and remove all flags on that line
    :Context:  buffer-menu
    :END:
*** Buffer-menu-unmark-all-buffers                                   :buffer:
    :PROPERTIES:
    :Command:  M-<DEL>
    :Description: Remove a particular flag from all lines
    :Context:  buffer-menu
    :END:
    This asks for a single character, and unmarks buffers marked with
    that character; typing ~<RET>~ removes all marks.
*** Buffer-menu-unmark-all                                           :buffer:
    :PROPERTIES:
    :Command:  U
    :Description: Remove all flags from all lines
    :Context:  buffer-menu
    :END:
*** Buffer-menu-not-modified                                    :buffer:edit:
    :PROPERTIES:
    :Command:  ~
    :Description: Mark buffer as unmodified
    :Context:  buffer-menu
    :END:
*** Buffer-menu-toggle-read-only                                :buffer:edit:
    :PROPERTIES:
    :Command:  %
    :Description: Toggle buffer's read-only status
    :Context:  buffer-menu
    :END:
*** Buffer-menu-visit-tags-table                                :buffer:mode:
    :PROPERTIES:
    :Command:  t
    :Description: Visit buffer as a tags table
    :Context:  buffer-menu
    :END:
*** quit-window                                                 :buffer:undo:
    :PROPERTIES:
    :Command:  q
    :Description: Quit the Buffer Menu
    :Context:  buffer-menu
    :END:
*** Buffer-menu-this-window                                  :buffer:display:
    :PROPERTIES:
    :Command:  <RET> OR f
    :Description: Select this line's buffer replacing Buffer Menu
    :Context:  buffer-menu
    :END:
*** Buffer-menu-other-window                                  :buffer:window:
    :PROPERTIES:
    :Command:  o
    :Description: Select this line's buffer in another window
    :Context:  buffer-menu
    :END:
*** Buffer-menu-switch-other-window                           :buffer:window:
    :PROPERTIES:
    :Command:  C-o
    :Description: Display this line's buffer in other window unselected
    :Context:  buffer-menu
    :END:
*** Buffer-menu-1-window                                             :buffer:
    :PROPERTIES:
    :Command:  1
    :Description: Select this line's buffer in full-frame window
    :Context:  buffer-menu
    :END:
*** Buffer-menu-2-window                                      :buffer:window:
    :PROPERTIES:
    :Command:  2
    :Description: Set up two windows on current frame involving this line
    :Context:  buffer-menu
    :END:
    ...
    ~*Buffer List*~) in the other.
*** Buffer-menu-bury                                                 :buffer:
    :PROPERTIES:
    :Command:  b
    :Description: Bury this line's buffer (i.e., move to end of list)
    :Context:  buffer-menu
    :END:
*** Buffer-menu-mark                                          :buffer:window:
    :PROPERTIES:
    :Command:  m
    :Description: Mark this line's buffer to display in other window on ~v~
    :Context:  buffer-menu
    :END:
    Mark this line's buffer to by displayed in another window if you
    exit with the ~v~ command. The display flag is indicated by the
    character '~>~' at the beginning of the line.
*** Buffer-menu-select                                               :buffer:
    :PROPERTIES:
    :Command:  v
    :Description: Select this buffer, display flagged buffers in windows
    :Context:  buffer-menu
    :END:
    Select this line's buffer, and also display in other windows any
    buffers flagged with the ~m~ command. If you have not flagged any
    buffers, this command is equivalent to ~1~.
*** tabulated-list-sort                                      :buffer:display:
    :PROPERTIES:
    :Command:  S
    :Description: Sort Buffer Menu entries according to column with point
    :Context:  buffer-menu
    :Argument: numeric
    :END:
    With a numeric argument /n/, sort according to the /n/-th column.
*** tabulated-list-narrow-current-column                     :buffer:display:
    :PROPERTIES:
    :Command:  {
    :Description: Narrow current column width by /n/ characters
    :Context:  buffer-menu
    :Argument: numeric
    :END:
    Numeric prefix argument defines /n/. Default is 1.
*** tabulated-list-widen-current-column                      :buffer:display:
    :PROPERTIES:
    :Command:  }
    :Description: Widen current column width by /n/ characters
    :Context:  buffer-menu
    :Argument: numeric
    :END:
    Numeric prefix argument defines /n/. Default is 1.
*** Buffer-menu-toggle-files-only                            :buffer:display:
    :PROPERTIES:
    :Command:  T
    :Description: Delete, or reinsert, lines for non-file buffers
    :Context:  buffer-menu
    :END:
*** revert-buffer                                       :buffer:edit:display:
    :PROPERTIES:
    :Command:  g
    :Description: update Buffer Menu
    :Context:  buffer-menu
    :END:
*** make-indirect-buffer                                             :buffer:
    :PROPERTIES:
    :Description: Create indirect buffer with given name from given buffer
    :Context:  navigating
    :END:
*** clonep-indirect-buffer                                           :buffer:
    :PROPERTIES:
    :Description: Create indirect buffer that is twin copy of curr buffer
    :Context:  anywhere
    :Argument: numeric
    :END:
    With a numeric argument, it prompts for the name of the indirect
    buffer; otherwise it uses the name of the current buffer, with a
    '<n>' suffix added.
*** clone-indirect-buffer-other-window                               :buffer:
    :PROPERTIES:
    :Command:  C-x 4 c
    :Description: Create clone indirect buffer and select in other window
    :Context:  navigating
    :Argument: numeric
    :END:
    With a numeric argument, it prompts for the name of the indirect
    buffer; otherwise it uses the name of the current buffer, with a
    '<n>' suffix added.
*** icomplete-force-complete-and-exit                         :complete:echo:
    :PROPERTIES:
    :Command:  C-j
    :Description: Select first completion in the list
    :Context:  icomplete
    :END:
*** icomplete-forward-completions                             :complete:echo:
    :PROPERTIES:
    :Command:  C-.
    :Description: Rotate icomplete list forward
    :Context:  icomplete
    :END:
*** icomplete-backward-completions                            :complete:echo:
    :PROPERTIES:
    :Command:  C-,
    :Description: Rotate icomplete list backwards
    :Context:  icomplete
    :END:
*** icomplete-force-complete                                  :complete:echo:
    :PROPERTIES:
    :Command:  C-M-i OR M-<TAB>
    :Description: Select first completion and do not exit
    :Context:  icomplete
    :END:
*** icomplete-mode                                            :mode:complete:
    :PROPERTIES:
    :Description: Toggle Icomplete mode
    :Context:  anywhere
    :END:
*** icomplete-fido-kill                                :complete:delete:kill:
    :PROPERTIES:
    :Command:  C-k
    :Description: Delete completion and thing behind completion
    :Context:  fido
    :END:
*** fido-mode                                                 :mode:complete:
    :PROPERTIES:
    :Description: Toggle Fido mode
    :Context:  navigating
    :END:
*** bs-show                                                  :buffer:display:
    :PROPERTIES:
    :Description: Make list of buffers like ~list-buffers~ but customizable
    :Context:  anywhere
    :END:
    For example, you can specify the list of buffer attributes to
    show, the minimum and maximum width of buffer name column, a
    regexp for names of buffer that will never be shown and those
    which will always be shown, etc.
*** ibuffer                                                  :buffer:display:
    :PROPERTIES:
    :Description: Make list of buffers in Dired-like fashion
    :Context:  anywhere
    :END:
*** bs-customize                                             :buffer:display:
    :PROPERTIES:
    :Description: Customize ~bs-show~ buffer list
    :Context:  anywhere
    :END:
** Multiple Windows
*** split-window-below                                               :window:
    :PROPERTIES:
    :Command:  C-x 2
    :Description: Split selected window into two windows one above other
    :Context:  navigating
    :Argument: numeric
    :END:
    After splitting, the selected window is the upper one, and the
    newly split-off window is below. Both windows have the same value
    of point as before, and display the same portion of the buffer (or
    as close to it as possible). If necessary, the window are scrolled
    to keep point on-screen. By default, the two windows each get half
    the height of the original window. A positive numeric argument
    specifies how many lines to give to the top window; A numeric
...
*** split-window-right                                               :window:
    ...
    :Argument: numeric
    :END:
    The left window is the selected one; the right window displays the
    same portion of the same buffer, and has the same value of
    point. A positive numeric argument specifies how many columns to
    give the left window; a negative numeric argument specifies how
    many columns to give the right window.
*** mouse-split-window-horizontally                                  :window:
    :PROPERTIES:
    :Command:  C-<mouse-2>
    :Description: Split window of mode line at location of click
    :Context:  mode-line
    :END:
*** other-window                                                 :window:nav:
    :PROPERTIES:
    :Command:  C-x o
    ...
    :Argument: numeric
    :END:
    When there are more than two windows, this command moves through
    all the windows in a cyclic order, generally top to bottom and
    left to right. After the rightmost and bottommost window, it goes
    back to the one at the upper left corner. A numeric argument means
    to move several steps in the cyclic order of windows. A negative
    argument moves around the cycle in the opposite order. When the
    minibuffer is active, the minibuffer window is the last window in
    the cycle; You can switch from the minibuffer window to one of the
    other windows, and later switch back and finish supplying the
    minibuffer argument that is requested.
*** scroll-other-window                                          :window:nav:
    :PROPERTIES:
    :Command:  C-M-v
    ...
    :Argument: numeric
    :END:
    With a numeric argument, /n/, you scroll next window by /n/ lines
*** scroll-other-window-down                                     :window:nav:
    :PROPERTIES:
    :Command:  C-M-S-v
    ...
    :END:
    With a numeric argument, /n/, you scroll next window by /n/ lines
*** mouse-set-point                                              :window:nav:
    :PROPERTIES:
    :Command:  <mouse-1>
    :Description: In text area, selects window and moves point to mouse
    :Context:  anywhere
    :END:
*** next-window-any-frame                                  :window:nav:frame:
    :PROPERTIES:
    :Description: Select next window, regardless of the frame
    :Context:  multiple-windows
    :END:
*** display-buffer                                    :window:display:buffer:
    :PROPERTIES:
    :Command:  C-x 4 C-o
    :Description: Display given buffer in some window without selecting
    :Context:  navigating
    :END:
*** find-file-other-window                          :window:files:nav:buffer:
    :PROPERTIES:
    :Command:  C-x 4 f
    :Description: Visit given file and select its buffer in another window
    :Context:  navigating
    :END:
*** dired-other-window                                  :window:files:buffer:
    :PROPERTIES:
    :Command:  C-x 4 d
    :Description: Select a dired buffer for given directory in other window
    :Context:  navigating
    :END:
*** compose-mail-other-window                                        :window:
    :PROPERTIES:
    :Command:  C-x 4 m
    :Description: Start composing a mail message in another window
    :Context:  anywhere
    :END:
*** xref-find-definitions-other-window                               :window:
    :PROPERTIES:
    :Command:  C-x 4 .
    :Description: Find the definition of an identifier in another window
    :Context:  prog
    :END:
*** find-file-read-only-other-window                           :window:files:
    :PROPERTIES:
    :Command:  C-x 4 r
    :Description: Visit given file read-only and select in other window
    :Context:  navigating
    :END:
*** delete-window                                             :window:delete:
    ...
    :Context:  navigating
    :END:
    Once a window is deleted, the space that it occupied is given to
    an adjacent window (but not the minibuffer window, even if that is
    active at the time). Deleting the window has no effect on the
    buffer it used to display; the buffer continues to exist, and you
    can still switch to it with ~C-x b~.
*** delete-other-window                                       :window:delete:
    :PROPERTIES:
    :Command:  C-x 1
    :Description: Delete all windows in selected frame except selected window
    :Context:  navigating
    :END:
    This command cannot be used while the minibuffer window is active;
    attempting to do so signals an error.)
*** kill-buffer-and-window                        :window:kill:buffer:delete:
    :PROPERTIES:
    :Command:  C-x 4 0
    :Description: Delete selected window and kill buffer in window
    :Context:  navigating
    :END:
*** delete-windows-on                                  :window:delete:buffer:
    :PROPERTIES:
    :Description: Delete windows showing the specified buffer
    :Context:  navigating
    :Argument: zero
    :END:
    With a prefix argument of zero, ~C-u 0~, this command deletes
    windows only on the current display's frames.
*** enlarge-window                                                   :window:
    :PROPERTIES:
    :Command:  C-x ^
    ...
    :Argument: numeric
    :END:
    With a positive numeric argument, this command increases the
    window height by that many lines; with a negative argument, it
    reduces the height by that many lines. If there are no vertically
    adjacent windows (i.e., the window is at the full frame height),
    that signals an error. The command also signals an error if you
    attempt to reduce the height of any window below a certain minimum
    number of lines, specified by the variable ~window-min-height~
    (the default is 4).
*** shrink-window-horizontally                                       :window:
    :PROPERTIES:
    :Command:  C-x {
    :Description: Make selected window narrower
    :Context:  multiple-windows
    :END:
*** enlarge-window-horizontally                                      :window:
    :PROPERTIES:
    :Command:  C-x }
    :Description: Make selected window wider
    :Context:  multiple-windows
    :END:
    This command signals an error if you attempt to reduce the width
    of any window below a certain minimum number of columns, specified
    by the variable ~window-min-width~ (the default is 10)
*** shrink-window-if-larger-than-buffer                              :window:
    :PROPERTIES:
    :Command:  C-x -
    :Description: Shrink this window if its buffer doesn't need all lines
    :Context:  multiple-windows
    :END:
*** balance-windows                                                  :window:
    :PROPERTIES:
    :Command:  C-x +
    :Description: Make all windows the same height
    :Context:  multiple-windows
    :END:
*** pop-to-buffer                                                    :window:
    :PROPERTIES:
    :Description: Display given buffer and select its window
    :Context:  navigating
    :END:
*** temp-buffer-resize-mode                              :mode:window:buffer:
    :PROPERTIES:
    :Description: Toggle Temp Buffer Resize mode
    :Context:  multiple-windows
    :END:
*** winner-mode                                         :undo:window:display:
    :PROPERTIES:
    :Description: Toggle Winner mode
    :Context:  navigating
    :END:
*** winner-undo                                         :window:undo:display:
    :PROPERTIES:
    :Command:  C-c <left>
    :Description: Undo last window configuration change
    :Context:  winner
    :END:
*** winner-redo                                         :window:undo:display:
    :PROPERTIES:
    :Command:  C-c <right>
    :Description: Redo changes to window you had just undone
    :Context:  winner
    :END:
*** windmove-default-keybindings                            :window:nav:keys:
    :PROPERTIES:
    :Description: Binds ~windmove-up~ to ~S-<up>~ etc.
    :Context:  anywhere
    :END:
    Running this command disables shift selection for those defaults.
*** windmove-up                                                  :window:nav:
    :PROPERTIES:
    :Command:  S-<up>
    :Description: Selects window immediately above current one
    :Context:  multiple-windows
    :END:
    Only bound to key if you run ~windmove-default-keybindings~
*** windmove-right                                               :window:nav:
    :PROPERTIES:
    :Command:  S-<right>
    :Description: Select window immediately right of current one
    :Context:  multiple-windows
    :END:
    Only bound to key if you run ~windmove-default-keybindings~
*** windmove-down                                                :window:nav:
    :PROPERTIES:
    :Command:  S-<down>
    :Description: Selects window immediately below current one
    :Context:  multiple-windows
    :END:
    Only bound to key if you run ~windmove-default-keybindings~
*** windmove-left                                                :window:nav:
    :PROPERTIES:
    :Command:  S-<left>
    :Description: Select window immediately left of current one
    :Context:  multiple-windows
    :END:
    Only bound to key if you run ~windmove-default-keybindings~
*** windmove-display-default-keybindings            :window:nav:display:keys:
    :PROPERTIES:
    :Description: Set up keybindings for creating new window directionally
    :Context:  anywhere
    :END:
*** windmove-display-up                                  :window:nav:display:
    :PROPERTIES:
    :Command:  S-M-<up>
    :Description: Display next buffer in window above current one
    :Context:  navigating
    :END:
    Only bound to key if you run ~windmove-display-default-keybindings~
*** windmove-display-right                               :window:nav:display:
    :PROPERTIES:
    :Command:  S-M-<right>
    :Description: Display next buffer in window to right of current one
    :Context:  navigating
    :END:
    Only bound to key if you run ~windmove-display-default-keybindings~
*** windmove-display-down                                :window:nav:display:
    :PROPERTIES:
    :Command:  S-M-<down>
    :Description: Display next buffer in window below current one
    :Context:  navigating
    :END:
    Only bound to key if you run ~windmove-display-default-keybindings~
*** windmove-display-left                                :window:nav:display:
    :PROPERTIES:
    :Command:  S-M-<left>
    :Description: Display next buffer in window to left of current one
    :Context:  navigating
    :END:
    Only bound to key if you run ~windmove-display-default-keybindings~
*** windmove-delete-default-keybindings                  :window:keys:delete:
    :PROPERTIES:
    :Description: Set up keybindings for directional window deletion
    :Context:  anywhere
    :END:
*** windmove-delete-up                                        :window:delete:
    :PROPERTIES:
    :Command:  C-x S-<up>
    :Description: Delete window above current one
    :Context:  multiple-windows
    :END:
*** windmove-delete-right                                     :window:delete:
    :PROPERTIES:
    :Command:  C-x S-<right>
    :Description: Delete window to right of current one
    :Context:  multiple-windows
    :END:
*** windmove-delete-down                                      :window:delete:
    :PROPERTIES:
    :Command:  C-x S-<down>
    :Description: Delete window below current one
    :Context:  multiple-windows
    :END:
*** windmove-delete-left                                      :window:delete:
    :PROPERTIES:
    :Command:  C-x S-<left>
    :Description: Delete window to left of current one
    :Context:  multiple-windows
    :END:
*** windmove-swap-states-default-keybindings            :window:keys:display:
    :PROPERTIES:
    :Description: Set up keybindings for directional window swap states
    :Context:  anywhere
    :END:
*** windmove-swap-states-up                                  :window:display:
    :PROPERTIES:
    :Command:  C-x @ s S-<up>
    :Description: Swap states with window above from current one
    :Context:  multiple-windows
    :END:
*** windmove-swap-states-right                               :window:display:
    :PROPERTIES:
    :Command:  C-x @ s S-<right>
    :Description: Swap states with window to right of current one
    :Context:  multiple-windows
    :END:
*** windmove-swap-states-down                                :window:display:
    :PROPERTIES:
    :Command:  C-x @ s S-<down>
    :Description: Swap states with window below from current one
    :Context:  multiple-windows
    :END:
*** windmove-swap-states-left                                :window:display:
    :PROPERTIES:
    :Command:  C-x @ s S-<left>
    :Description: Swap states with window to left of current one
    :Context:  multiple-windows
    :END:
*** scroll-all-mode                                              :window:nav:
    :PROPERTIES:
    :Description: Toggle Scroll All mode
    :Context:  multiple-windows
    :END:
*** global-tab-line-mode                              :window:buffer:display:
    :PROPERTIES:
    :Description: Toggle Global Tab Line mode
    :Context:  navigating
    :END:
** Frames and Graphical Displays
*** delete-frame                                               :frame:delete:
    :PROPERTIES:
    :Command:  C-x 5 0
...
* Variables
  :PROPERTIES:
  :COLUMNS:  %20ITEM(Var) %20TAGS(Tags) %20Description
  :ID:       vars
  :END:
** The Organization of the Screen
...
*** inhibit-startup-screen                                   :startup:buffer:
    :PROPERTIES:
    :Description: Non-nil inhibits the startup screen.
    :END:
...
*** initial-buffer-choice                                    :startup:buffer:
    :PROPERTIES:
    :Description: Buffer to show after starting Emacs.
    :END:
...
*** confirm-kill-emacs                                           :kill:frame:
    :PROPERTIES:
    :Description: How to ask for confirmation when leaving Emacs.
    :END:
...
*** confirm-kill-processes                                             :kill:
    :PROPERTIES:
    :Description: Non-nil if Emacs should confirm killing processes on exit.
    :END:
...
*** read-quoted-char-radix                                        :edit:char:
    :PROPERTIES:
    :Description: Radix for C-q and other uses of ‘read-quoted-char’.
    :END:
    To use decimal or hexadecimal when running ~quoted-insert~
    command.
*** line-move-visual                                             :nav:window:
    :PROPERTIES:
    :Description: When non-nil, ‘line-move’ moves point by visual lines.
    :END:
...
*** track-eol                                                    :nav:buffer:
    :PROPERTIES:
    :Description: Non-nil means vertical motion starting at end of line keeps to ends of lines.
    :END:
...
*** next-line-add-newlines                                  :nav:edit:buffer:
    :PROPERTIES:
    :Description: If non-nil, ‘next-line’ inserts newline to avoid ‘end of buffer’ error.
    :END:
...
*** minibuffer-eldef-shorten-default                    :buffer:display:echo:
    :PROPERTIES:
    :Description: If non-nil, shorten "(default ...)" to "[...]" in minibuffer prompts.
    :END:
...
*** insert-default-directory                            :buffer:display:echo:
    :PROPERTIES:
    :Description: Non-nil means when reading a filename start with default dir in minibuffer.
    :END:
...
*** resize-mini-windows                                 :buffer:display:echo:
    :PROPERTIES:
    :Description: How to resize mini-windows (the minibuffer and the echo area).
    :END:
...
*** max-mini-window-height                              :buffer:display:echo:
    :PROPERTIES:
    :Description: Maximum height for resizing mini-windows (the minibuffer and the echo area).
    :END:
...
*** enable-recursive-minibuffers                        :buffer:command:echo:
    :PROPERTIES:
    :Description: Non-nil means to allow minibuffer commands while in the minibuffer.
    :END:
...
*** confirm-nonexistent-file-or-buffer                  :buffer:command:echo:
    :PROPERTIES:
    :Description: Whether confirmation is requested before visiting a new file or buffer.
    :END:
...
*** completion-styles                                         :list:complete:
    :PROPERTIES:
    :Description: List of completion styles to use.
    :END:
    See current list and list options defined in:
    [[How Completion Alternatives Are Chosen]]
*** read-file-name-completion-ignore-case        :files:complete:buffer:echo:
    :PROPERTIES:
    :Description: Non-nil means when reading a file name completion ignores case.
    :END:
...
*** read-buffer-completion-ignore-case                 :buffer:complete:echo:
    :PROPERTIES:
    :Description: Non-nil means completion ignores case when reading a buffer name.
    :END:
...
*** completion-ignored-extensions           :files:complete:list:buffer:echo:
    :PROPERTIES:
    :Description: Completion ignores file names ending in any string in this list.
    :END:
...
*** completion-auto-help                  :complete:inform:helps:buffer:echo:
    :PROPERTIES:
    :Description: Non-nil means automatically provide help for invalid completion input.
    :END:
...
*** completion-cycle-threshold            :complete:inform:helps:buffer:echo:
    :PROPERTIES:
    :Description: Number of completion candidates below which cycling is used.
    :END:
...
*** history-length                                         :buffer:hist:echo:
    :PROPERTIES:
    :Description: Maximum length of history lists before truncation takes place.
    :END:
...
*** history-delete-duplicates                                          :hist:
    :PROPERTIES:
    :Description: Non-nil means to delete duplicates in history.
    :END:
...
*** isearch-resume-in-command-history                          :command:hist:
    :PROPERTIES:
    :Description: If non-nil, ‘isearch-resume’ commands are added to the command history.
    :END:
    You can make incremental search commands appear in the history by
    setting this variable to a non-nil value.
*** command-history                                       :command:hist:list:
    :PROPERTIES:
    :Description: List of recent commands that read arguments from terminal.
    :END:
...
*** suggest-key-bindings                                :inform:command:keys:
    :PROPERTIES:
    :Description: Non-nil means show the equivalent key-binding when M-x command has one.
    :END:
...
*** extended-command-suggest-shorter                         :inform:command:
    :PROPERTIES:
    :Description: If non-nil, show a shorter M-x invocation when there is one.
    :END:
...
*** apropos-do-all                                           :search:command:
    :PROPERTIES:
    :Description: Non-nil means apropos commands will search more extensively.
    :END:
...
*** apropos-sort-by-scores                                   :search:display:
    :PROPERTIES:
    :Description: Non-nil means sort matches by scores; best match is shown first.
    :END:
...
*** help-at-pt-display-when-idle                   :word:inform:display:echo:
    :PROPERTIES:
    :Description: Automatically show local help on point-over.
    :END:
...
*** highlight-nonselected-windows                     :display:region:window:
    :PROPERTIES:
    :Description: Non-nil means highlight region even in nonselected windows.
    :END:
...
*** use-empty-active-region                                  :region:command:
    :PROPERTIES:
    :Description: Whether "region-aware" commands should act on empty regions.
    :END:
...
*** delete-active-region                            :region:edit:delete:kill:
    :PROPERTIES:
    :Description: Whether single-char deletion commands delete an active region.
    :END:
...
*** mark-even-if-inactive                                    :region:command:
    :PROPERTIES:
    :Description: Non-nil means you can use the mark even when inactive.
    :END:
...
*** set-mark-command-repeat-pop                        :nav:region:keys:hist:
    :PROPERTIES:
    :Description: Non-nil means repeating C-SPC after popping mark pops it again.
    :END:
...
*** mark-ring-max                                               :region:hist:
    :PROPERTIES:
    :Description: Maximum size of mark ring.  Start discarding off end if gets this big.
    :END:
...
*** global-mark-ring-max                                        :region:hist:
    :PROPERTIES:
    :Description: Maximum size of global mark ring.  Start discarding off end if gets this big.
    :END:
    The length of the global mark ring is controlled by this variable,
    and is 16 by default.
*** shift-select-mode                                       :nav:region:mode:
    :PROPERTIES:
    :Description: When non-nil, shifted motion keys activate the mark momentarily.
    :END:
...
*** kill-whole-line                                        :kill:delete:line:
    :PROPERTIES:
    :Description: If non-nil, ‘kill-line’ with no arg at start of line kills the whole line.
    :END:
...
*** kill-read-only-ok                                                  :kill:
    :PROPERTIES:
    :Description: Non-nil means don’t signal an error for killing read-only text.
    :END:
...
*** kill-do-not-save-duplicates                                   :kill:hist:
    :PROPERTIES:
    :Description: If non-nil, don’t add a string to ‘kill-ring’ if it duplicates the last one.
    :END:
...
*** save-interprogram-paste-before-kill                           :kill:hist:
    :PROPERTIES:
    :Description: Save existing clipboard text into kill ring before replacing it.
    :END:
...
*** yank-pop-change-selection                                     :kill:hist:
    :PROPERTIES:
    :Description: Whether rotating the kill ring changes the window system selection.
    :END:
    Normally rotating the kill ring with ~M-y~ (~yank-pop~) does not
...
*** select-enable-clipboard                                            :kill:
    :PROPERTIES:
    :Description: Non-nil means cutting and pasting uses the clipboard.
    :END:
    To prevent kill and yank commands from accessing the clipboard,
    change this variable to ~nil~.
*** x-select-enable-clipboard-manager                                  :kill:
    :PROPERTIES:
    :Description: Whether to enable X clipboard manager support.
    :END:
...
*** select-enable-primary                                              :kill:
    :PROPERTIES:
    :Description: Non-nil means cutting and pasting uses the primary selection.
    :END:
...
*** mouse-drag-copy-region                                      :kill:region:
    :PROPERTIES:
    :Description: If non-nil, copy to kill-ring upon mouse adjustments of the region.
    :END:
    If non-nil, copy to kill-ring upon mouse adjustments of the
    region.
*** select-active-regions                                            :region:
    :PROPERTIES:
    :Description: If non-nil, an active region automatically sets the primary selection.
    :END:
...
*** mouse-yank-at-point                                                :edit:
    :PROPERTIES:
    :Description: If non-nil, mouse yank commands yank at point instead of at click.
    :END:
...
*** register-preview-delay                     :register:command:echo:window:
    :PROPERTIES:
    :Description: If non-nil, time to wait in seconds before popping up a preview window.
    :END:
...
*** register-separator                                             :register:
    :PROPERTIES:
    :Description: Register containing the text to put between collected texts, or nil if none.
    :END:
...
*** bookmark-save-flag                                     :files:nav:buffer:
    :PROPERTIES:
    :Description: Controls when Emacs saves bookmarks to a file.
    :END:
...
*** bookmark-default-file                                  :files:nav:buffer:
    :PROPERTIES:
    :Description: File in which to save bookmarks by default.
    :END:
    The value of this variable specifies the file in which to save
    bookmarks by default.
*** bookmark-search-size                                    :buffer:hist:nav:
    :PROPERTIES:
    :Description: Length of the context strings recorded on either side of a bookmark.
    :END:
...
*** next-screen-context-lines                                   :display:nav:
    :PROPERTIES:
    :Description: Number of lines of continuity when scrolling by screenfuls.
    :END:
...
*** scroll-error-top-bottom                                     :display:nav:
    :PROPERTIES:
    :Description: Move point to top/bottom of buffer before signaling a scrolling error.
    :END:
...
*** scroll-preserve-screen-position                             :display:nav:
    :PROPERTIES:
    :Description: Controls if scroll commands move point to keep its screen position unchanged.
    :END:
...
*** recenter-positions                      :list:window:display:nav:command:
    :PROPERTIES:
    :Description: Cycling order for ‘recenter-top-bottom’.
    :END:
...
*** recenter-redisplay                                       :display:window:
    :PROPERTIES:
    :Description: Non-nil means ‘recenter’ redraws entire frame.
    :END:
...
*** scroll-conservatively                                    :display:window:
    :PROPERTIES:
    :Description: Scroll up to this many lines, to bring point back on screen.
    :END:
...
*** scroll-step                                              :display:window:
    :PROPERTIES:
    :Description: The number of lines to try scrolling a window by when point moves out.
    :END:
...
*** scroll-up-aggressively                                   :display:window:
    :PROPERTIES:
    :Description: How far to scroll windows upward.
    :END:
...
*** scroll-down-aggressively                                 :display:window:
    :PROPERTIES:
    :Description: How far to scroll windows downward.
    :END:
    This variable directly specifies the vertical position of point
    after scrolling. The value of this variable should be either ~nil~
    (the default), or a floating point number /f/ between 0 and 1. The
...
*** scroll-margin                                            :display:window:
    :PROPERTIES:
    :Description: Number of lines of margin at the top and bottom of a window.
    :END:
...
*** maximum-scroll-margin                                    :display:window:
    :PROPERTIES:
    :Description: Maximum effective value of ‘scroll-margin’.
    :END:
...
*** auto-hscroll-mode                                        :display:window:
    :PROPERTIES:
    :Description: Allow or disallow automatic horizontal scrolling of windows.
    :END:
...
*** hscroll-margin                                           :display:window:
    :PROPERTIES:
    :Description: How many columns away from the window edge point is allowed to get
    :END:
...
*** hscroll-step                                             :display:window:
    :PROPERTIES:
    :Description: How many columns to scroll the window when point gets too close to the edge.
    :END:
...
*** list-colors-sort                                   :display:helps:window:
    :PROPERTIES:
    :Description: Color sort order for ‘list-colors-display’.
    :END:
...
*** text-scale-mode-step                                            :display:
    :PROPERTIES:
    :Description: Scale factor used by ‘text-scale-mode’.
    :END:
...
*** global-font-lock-mode                       :buffer:display:mode:startup:
    :PROPERTIES:
    :Description: Non-nil if Global Font-Lock mode is enabled.
    :END:
...
*** font-lock-maximum-decoration                               :display:mode:
    :PROPERTIES:
    :Description: Maximum decoration level for fontification.
    :END:
...
*** fringe-mode                                   :window:frame:mode:display:
    :PROPERTIES:
    :Description: Default appearance of fringes on all frames.
    :END:
...
*** overflow-newline-into-fringe                             :display:window:
    :PROPERTIES:
    :Description: Non-nil means that newline may flow into the right fringe.
    :END:
...
*** display-fill-column-indicator                     :display:window:buffer:
    :PROPERTIES:
    :Description: Non-nil means display the fill column indicator.
    :END:
...
*** display-fill-column-indicator-character      :display:window:buffer:char:
    :PROPERTIES:
    :Description: Character to draw the indicator when ‘display-fill-column-indicator’ is non-nil.
    :END:
...
*** display-fill-column-indicator-column              :display:window:buffer:
    :PROPERTIES:
    :Description: Column for indicator when ‘display-fill-column-indicator’ is non-nil.
    :END:
...
*** indicate-buffer-boundaries                                :display:frame:
    :PROPERTIES:
    :Description: Visually indicate buffer boundaries and scrolling.
    :END:
...
*** show-trailing-whitespace                                        :display:
    :PROPERTIES:
    :Description: Non-nil means highlight trailing whitespace.
    :END:
...
*** delete-trailing-lines                               :edit:command:buffer:
    :PROPERTIES:
    :Description: If non-nil, M-x delete-trailing-whitespace deletes trailing lines.
    :END:
...
*** indicate-empty-lines                              :display:window:buffer:
    :PROPERTIES:
    :Description: Visually indicate empty lines after the buffer end.
    :END:
...
*** selective-display-ellipses                          :display:buffer:line:
    :PROPERTIES:
    :Description: Non-nil means display ... on previous line when a line is invisible.
    :END:
...
*** column-number-indicator-zero-based                       :display:inform:
    :PROPERTIES:
    :Description: When non-nil, mode line displays column numbers zero-based.
    :END:
...
*** line-number-display-limit                           :line:display:inform:
    :PROPERTIES:
    :Description: Maximum buffer size for which line number should be displayed.
    :END:
...
*** line-number-display-limit-width                     :line:display:inform:
    :PROPERTIES:
    :Description: Maximum line width (in characters) for line number display.
    :END:
...
*** display-time-24hr-format                                 :display:inform:
    :PROPERTIES:
    :Description: Non-nil indicates time should be displayed as hh:mm, 0 <= hh <= 23.
    :END:
...
*** display-time-use-mail-icon                               :display:inform:
    :PROPERTIES:
    :Description: Non-nil means use an icon as mail indicator on a graphic display.
    :END:
...
*** display-time-mail-file                             :display:inform:files:
    :PROPERTIES:
    :Description: File name of mail inbox file, for indicating existence of new mail.
    :END:
...
*** display-time-mail-directory                        :display:inform:files:
    :PROPERTIES:
    :Description: Name of mail inbox directory, for indicating existence of new mail.
    :END:
...
*** display-battery-mode                                :display:inform:mode:
    :PROPERTIES:
    :Description: Non-nil if Display-Battery mode is enabled.
    :END:
...
*** mode-line-in-non-selected-windows                               :display:
    :PROPERTIES:
    :Description: Non-nil means to use ‘mode-line-inactive’ face in non-selected windows.
    :END:
...
*** eol-mnemonic-unix                                          :display:char:
    :PROPERTIES:
    :Description: String displayed in mode line for UNIX-like (LF) end-of-line format.
    :END:
...
*** eol-mnemonic-mac                                           :display:char:
    :PROPERTIES:
    :Description: String displayed in mode line for MAC-like (CR) end-of-line format.
    :END:
    This variable determines what character is used to indicate in the
...
*** eol-mnemonic-undecided                                     :display:char:
    :PROPERTIES:
    :Description: String displayed in mode line when end-of-line format is not yet determined.
    :END:
    This variable determines what character is used to indicate in the
...
*** tab-width                                                  :display:char:
    :PROPERTIES:
    :Description: Distance between tab stops (for display of tab characters), in columns.
    :END:
...
*** ctl-arrow                                           :display:char:coding:
    :PROPERTIES:
    :Description: Non-nil means display control chars with uparrow.
    :END:
...
*** nobreak-char-display                                :display:char:coding:
    :PROPERTIES:
    :Description: Control highlighting of non-ASCII space and hyphen chars.
    :END:
...
*** glyphless-char-display-control                      :display:char:coding:
    :PROPERTIES:
    :Description: List of directives to control display of glyphless characters.
    :END:
...
*** text-quoting-style                                         :display:char:
    :PROPERTIES:
    :Description: Style to use for single quotes in help and messages.
    :END:
...
*** visible-cursor                                              :display:nav:
    :PROPERTIES:
    :Description: Non-nil means to make the cursor very visible.
    :END:
...
*** cursor-type                                                 :display:nav:
    :PROPERTIES:
    :Description: Cursor to use when this buffer is in the selected window.
    :END:
...
*** blink-cursor-blinks                                         :display:nav:
    :PROPERTIES:
    :Description: How many times to blink before using a solid cursor on NS, X, and MS-Windows.
    :END:
...
*** blink-cursor-mode                                           :display:nav:
    :PROPERTIES:
    :Description: Non-nil if Blink-Cursor mode is enabled.
    :END:
    To disable cursor blinking altogether, change this variable to
    ~nil~.
*** blink-cursor-alist                                     :display:nav:list:
    :PROPERTIES:
    :Description: Alist specifying how to blink the cursor off.
    :END:
    You can change how the cursor looks when it blinks off by
...
*** x-stretch-cursor                                            :display:nav:
    :PROPERTIES:
    :Description: Non-nil means draw block cursor as wide as the glyph under it.
    :END:
    Some characters, such as tab characters, are extra wide. When the
    cursor is positioned over such a character, it is normally drawn
    with the default character width. You can make the cursor stretch
...
*** cursor-in-non-selected-windows                            :display:frame:
    :PROPERTIES:
    :Description: Non-nil means show a cursor in non-selected windows.
    :END:
    To turn off cursors in non-selected windows, change this variable
    to ~nil~.
*** truncate-lines                                             :display:line:
    :PROPERTIES:
    :Description: Non-nil means do not display continuation lines.
    :END:
...
*** visual-line-fringe-indicators                       :display:mode:window:
    :PROPERTIES:
    :Description: How fringe indicators are shown for wrapped lines in ‘visual-line-mode’.
    :END:
...
*** display-line-numbers                         :display:buffer:window:line:
    :PROPERTIES:
    :Description: Non-nil means display line numbers.
    :END:
    If you want to have Emacs display line numbers for every line in
...
*** display-line-numbers-current-absolute        :display:buffer:window:line:
    :PROPERTIES:
    :Description: Non-nil means display absolute number of current line.
    :END:
...
*** display-line-numbers-widen                   :display:buffer:window:line:
    :PROPERTIES:
    :Description: Non-nil means display line numbers disregarding any narrowing.
    :END:
...
*** display-line-numbers-offset                  :display:buffer:window:line:
    :PROPERTIES:
    :Description: A signed integer added to each absolute line number.
    :END:
...
*** display-line-numbers-width-start             :display:window:buffer:line:
    :PROPERTIES:
    :Description: Not documented as a variable.
    :END:
...
*** display-line-numbers-width                   :display:window:buffer:line:
    :PROPERTIES:
    :Description: Minimum width of space reserved for line number display.
    :END:
...
*** display-line-numbers-major-tick              :display:window:buffer:line:
    :PROPERTIES:
    :Description: If an integer N > 0, highlight line number of every Nth line.
    :END:
...
*** visible-bell                                             :display:window:
    :PROPERTIES:
    :Description: Non-nil means try to flash the frame to represent a bell.
    :END:
...
*** echo-keystrokes                                       :keys:echo:display:
    :PROPERTIES:
    :Description: Nonzero means echo unfinished commands after this many seconds of pause.
    :END:
...
*** display-hourglass                                         :display:frame:
    :PROPERTIES:
    :Description: Non-nil means show an hourglass pointer, when Emacs is busy.
    :END:
...
*** hourglass-delay                                           :display:frame:
    :PROPERTIES:
    :Description: Seconds to wait before displaying an hourglass pointer when Emacs is busy.
    :END:
...
*** make-pointer-invisible                                    :display:frame:
    :PROPERTIES:
    :Description: If non-nil, make pointer invisible while typing.
    :END:
...
*** underline-minimum-offset                              :char:display:edit:
    :PROPERTIES:
    :Description: Minimum distance between baseline and underline.
    :END:
...
*** x-underline-at-descent-line                           :char:display:edit:
    :PROPERTIES:
    :Description: Non-nil means to draw the underline at the same place as the descent line.
    :END:
...
*** overline-margin                                       :char:display:edit:
    :PROPERTIES:
    :Description: Space between overline and text, in pixels.
    :END:
...
*** display-raw-bytes-as-hex                            :char:coding:display:
    :PROPERTIES:
    :Description: Non-nil means display raw bytes in hexadecimal format.
    :END:
...
*** isearch-lazy-highlight                            :search:display:buffer:
    :PROPERTIES:
    :Description: Controls the lazy-highlighting during incremental search.
    :END:
...
*** search-ring-max                                                  :search:
    :PROPERTIES:
    :Description: Maximum length of search ring before oldest elements are thrown away.
    :END:
...
*** search-upper-case                                                :search:
    :PROPERTIES:
    :Description: If non-nil, upper case chars disable case fold searching.
    :END:
...
*** search-exit-option                                  :search:edit:command:
    :PROPERTIES:
    :Description: Defines what control characters do in incremental search.
    :END:
...
*** isearch-allow-prefix                                     :search:command:
    :PROPERTIES:
    :Description: Whether prefix arguments are allowed during incremental search.
    :END:
...
*** isearch-allow-scroll                                     :search:command:
    :PROPERTIES:
    :Description: Whether scrolling is allowed during incremental search.
    :END:
...
*** isearch-yank-on-move                                 :search:region:edit:
    :PROPERTIES:
    :Description: Motion keys yank text to the search string while you move the cursor.
    :END:
...
*** regexp-search-ring-max                                           :search:
    :PROPERTIES:
    :Description: Maximum length of regexp search ring before oldest elements are thrown away.
    :END:
...
*** search-whitespace-regexp                                    :search:char:
    :PROPERTIES:
    :Description: If non-nil, regular expression to match a sequence of whitespace chars.
    :END:
...
*** case-fold-search                                     :search:buffer:char:
    :PROPERTIES:
    :Description: Non-nil if searches and matches should ignore case.
    :END:
...
*** search-default-mode                                         :search:char:
    :PROPERTIES:
    :Description: Default mode to use when starting isearch.
    :END:
...
*** char-fold-symmetric                                         :search:char:
    :PROPERTIES:
    :Description: Non-nil means char-fold searching treats equivalent chars the same.
    :END:
...
*** char-fold-include                                           :search:char:
    :PROPERTIES:
    :Description: Additional character foldings to include.
    :END:
    You can add new character foldings using this variable.
*** char-fold-exclude                                           :search:char:
    :PROPERTIES:
    :Description: Character foldings to remove from default decompositions.
    :END:
    You can remove existing character foldings using this variable.
*** replace-lax-whitespace                                      :search:char:
    :PROPERTIES:
    :Description: Non-nil means ‘query-replace’ matches a sequence of whitespace chars.
    :END:
...
*** replace-regexp-lax-whitespace                               :search:char:
    :PROPERTIES:
    :Description: Non-nil means ‘query-replace-regexp’ matches a sequence of whitespace chars.
    :END:
    Unlike incremental search, the replacement commands do not use lax
...
*** case-replace                                                :search:char:
    :PROPERTIES:
    :Description: Non-nil means ‘query-replace’ should preserve case in replacements.
    :END:
    If upper-case letters are used in the replacement string, they
    remain upper case every time that text is inserted. If upper-case
    letters are used in the first argument, the second argument is
...
*** replace-char-fold                                           :search:char:
    :PROPERTIES:
    :Description: Non-nil means replacement commands should do character folding in matches.
    :END:
...
*** query-replace-from-to-separator                          :search:display:
    :PROPERTIES:
    :Description: String that separates FROM and TO in the history of replacement pairs.
    :END:
    You can reuse earlier replacements with the commands
    ~query-replace~ or ~query-replace-regexp~. When either these
    commands prompts for the search string, use ~M-p~ and ~M-n~ to
...
*** query-replace-highlight                                  :search:display:
    :PROPERTIES:
    :Description: Non-nil means to highlight matches during query replacement.
    :END:
...
*** query-replace-lazy-highlight                             :search:display:
    :PROPERTIES:
    :Description: Controls the lazy-highlighting during query replacements.
    :END:
    The commands ~query-replace~ and ~query-replace-regexp~ highlight
...
*** query-replace-show-replacement                           :search:display:
    :PROPERTIES:
    :Description: Non-nil means show substituted replacement text in the minibuffer.
    :END:
...
*** query-replace-skip-read-only                                 :search:nav:
    :PROPERTIES:
    :Description: Non-nil means ‘query-replace’ and friends ignore read-only matches.
    :END:
...
*** search-invisible                                                 :search:
    :PROPERTIES:
    :Description: If t incremental search/query-replace can match hidden text.
    :END:
...
*** list-matching-lines-default-context-lines  :search:display:inform:window:
    :PROPERTIES:
    :Description: Default number of context lines included around ‘list-matching-lines’ matches.
    :END:
...
*** list-matching-lines-jump-to-current-line   :search:inform:window:display:
    :PROPERTIES:
    :Description: If non-nil, M-x list-matching-lines shows the current line highlighted.
    :END:
...
*** search-highlight                                         :search:display:
    :PROPERTIES:
    :Description: Non-nil means incremental search highlights the current match.
    :END:
...
*** lazy-highlight-initial-delay                              :search:display:
    :PROPERTIES:
    :Description: Seconds to wait before beginning to lazily highlight all matches.
    :END:
    Time in seconds to wait before highlighting visible matches.
*** lazy-highlight-interval                                  :search:display:
    :PROPERTIES:
    :Description: Seconds between lazily highlighting successive matches.
    :END:
    Time in seconds between highlighting successive matches.
*** lazy-highlight-max-at-a-time                             :search:display:
    :PROPERTIES:
    :Description: Maximum matches to highlight at a time (for ‘lazy-highlight’).
    :END:
...
*** isearch-lazy-count                                :search:display:inform:
    :PROPERTIES:
    :Description: Show match numbers in the search prompt.
    :END:
    Show the current match number and the total number of matches in
    the search prompt
*** lazy-count-prefix-format                          :search:display:inform:
    :PROPERTIES:
    :Description: Format of the current/total number of matches for the prompt prefix.
    :END:
...
*** lazy-count-suffix-format                          :search:display:inform:
    :PROPERTIES:
    :Description: Format of the current/total number of matches for the prompt suffix.
    :END:
...
*** search-nonincremental-instead                                    :search:
    :PROPERTIES:
    :Description: If non-nil, do a nonincremental search instead of exiting immediately.
    :END:
...
*** isearch-hide-immediately                                 :search:display:
    :PROPERTIES:
    :Description: If non-nil, re-hide an invisible match right away.
    :END:
...
*** search-slow-speed                                                :search:
    :PROPERTIES:
    :Description: Highest terminal speed at which to use "slow" style incremental search.
    :END:
...
*** search-slow-window-lines                                 :search:display:
    :PROPERTIES:
    :Description: Number of lines in slow search display windows.
    :END:
    Emacs provides a special display mode for slow terminals, whereby
    search pops up a separate small window and displays the text
...
*** undo-limit                                                         :undo:
    :PROPERTIES:
    :Description: Keep no more undo information once it exceeds this size.
    :END:
...
*** undo-strong-limit                                                  :undo:
    :PROPERTIES:
    :Description: Don’t keep more than this much size of undo information.
    :END:
    You can specify how much undo information to keep by setting this
...
*** undo-outer-limit                                                   :undo:
    :PROPERTIES:
    :Description: Outer limit on size of undo information for one command.
    :END:
    You can specify how much undo information to keep by setting this
...
*** ispell-local-dictionary                                          :inform:
    :PROPERTIES:
    :Description: If non-nil, the dictionary to be used for Ispell commands in this buffer.
    :END:
...
*** ispell-dictionary                                                :inform:
    :PROPERTIES:
    :Description: Default dictionary to use if ‘ispell-local-dictionary’ is nil.
    :END:
...
*** ispell-personal-dictionary                                       :inform:
    :PROPERTIES:
    :Description: File name of your personal spelling dictionary, or nil.
    :END:
...
*** ispell-complete-word-dict                                        :inform:
    :PROPERTIES:
    :Description: Plain word-list dictionary used for word completion if
    :END:
...
*** kmacro-execute-before-append                                      :macro:
    :PROPERTIES:
    :Description: Controls whether appending to a macro starts by executing the macro.
    :END:
...
*** kmacro-ring-max                                                   :macro:
    :PROPERTIES:
    :Description: Maximum number of keyboard macros to save in macro ring.
    :END:
...
*** default-directory                                            :files:echo:
    :PROPERTIES:
    :Description: Local in buffer emacs-manual.org; global value is nil
    :END:
...
*** large-file-warning-threshold                               :files:inform:
    :PROPERTIES:
    :Description: Maximum size of file above which a confirmation is requested.
    :END:
...
*** find-file-run-dired                                   :files:mode:inform:
    :PROPERTIES:
    :Description: Non-nil means allow ‘find-file’ to visit directories.
    :END:
    If you visit a file that is actually a directory, Emacs invokes
...
*** find-file-wildcards                                          :files:edit:
    :PROPERTIES:
    :Description: Non-nil means file-visiting commands should handle wildcards.
    :END:
    If the file name you specify contains shell-style wildcard
    characters, Emacs visits all the files that match it. (On
    case-insensitive filesystems, Emacs matches the wildcards
...
*** save-some-buffers-default-predicate                          :files:edit:
    :PROPERTIES:
    :Description: Default predicate for ‘save-some-buffers’.
    :END:
    You can customize the value of this variable to control which
    buffers Emacs will ask about.
*** make-backup-files                                            :files:undo:
    :PROPERTIES:
    :Description: Non-nil means make a backup of a file the first time it is saved.
    :END:
...
*** vc-make-backup-files                                              :files:
    :PROPERTIES:
    :Description: If non-nil, backups of registered files are made as with other files.
    :END:
...
*** backup-enable-predicate                                           :files:
    :PROPERTIES:
    :Type:     global
    :Description: Predicate that looks at a file name and decides whether to make backups.
    :END:
...
*** temporary-file-directory                                          :files:
    :PROPERTIES:
    :Description: The directory for writing temporary files.
    :END:
    The directory for writing temporary files
*** small-temporary-file-directory                                    :files:
    :PROPERTIES:
    :Description: The directory for writing small temporary files.
    :END:
...
*** backup-directory-alist                                            :files:
    :PROPERTIES:
    :Description: Alist of filename patterns and backup directory names.
    :END:
...
*** version-control                                                   :files:
    ...
    :Description: Control use of version numbers for backup files.
    :END:
...
*** make-backup-file-name-function                                    :files:
    :PROPERTIES:
    :Description: A function that ‘make-backup-file-name’ uses to create backup file names.
    :END:
...
*** kept-old-versions                                                 :files:
    :PROPERTIES:
    :Description: Number of oldest versions to keep when a new numbered backup is made.
    :END:
...
*** kept-new-versions                                                 :files:
    :PROPERTIES:
    :Description: Number of newest versions to keep when a new numbered backup is made.
    :END:
    ...
    value is 2.
*** delete-old-versions                                        :files:delete:
    :PROPERTIES:
    :Description: If t, delete excess backup versions silently.
    :END:
...
*** backup-by-copying                                                 :files:
    :PROPERTIES:
    :Description: Non-nil means always use copying to create backup files.
    :END:
    If this variable is non-nil (the default is ~nil~), use copying.
*** backup-by-copying-when-linked                                     :files:
    :PROPERTIES:
    :Description: Non-nil means use copying to create backups for files with multiple names.
    :END:
    If ~backup-by-copying~ is ~nil~ and this variable is non-nil (the
    default is ~nil~), and the file has multiple names, use copying.
*** backup-by-copying-when-mismatch                                   :files:
    :PROPERTIES:
    :Description: Non-nil means create backups by copying if this preserves owner or group.
    :END:
    If ~backup-by-copying~ and ~backup-by-copying-when-linked~ are
    both ~nil~ and this variable is non-nil (the default is ~t~), and
    renaming would change the file's owner or group, use copying.

    If you change this variable to ~nil~, Emacs checks the numberic
    user-id of the file's owner and the numberic group-id of the
    file's group. If either is no greater than
    ~backup-by-copying-when-privileged-mismatch~, then it behaves as
    though this variable is non-nil anyway.

    Otherwise, renaming is the default choice.
*** require-final-newline                                        :files:char:
    :PROPERTIES:
    :Description: Whether to add a newline automatically at the end of the file.
    :END:
    If the value of this variable is ~t~, saving or writing a file
    silently puts a newline at the end if there isn't already one
    there. If the value is ~visit~, Emacs adds a newline at the end of
    any file that doesn't have one, just after it visits the
    file. (This marks the buffer as modified, and you can undo it.) If
    the value is ~visit-save~, Emacs adds such newlines both on
    visiting and on saving. If the value is ~nil~, Emacs leaves the
    end of the file unchanged; any other non-nil value means Emacs
    asks you whether to add a newline. The default is ~nil~.
*** mode-require-final-newline                                   :files:char:
    :PROPERTIES:
    :Description: Whether to add a newline at end of file, in certain major modes.
    :END:
    Some major modes are designed for specific kinds of files that are
    always supposed to end in newlines. such major modes set the
    variable ~require-final-newline~ to the value of this variable,
    which defaults to ~t~. By setting this variable, you can control
    how these modes handle final newlines.
*** write-region-inhibit-fsync                                        :files:
    :PROPERTIES:
    :Description: Non-nil means don’t call fsync in ‘write-region’.
    :END:
    This variable controls whether Emacs invokes ~fsync~ after saving
    a file. The variable's default value is ~nil~ when Emacs is
    interactive, and ~t~ when Emacs runs in batch mode.
*** create-lockfiles                                                  :files:
    :PROPERTIES:
    :Description: Non-nil means use lockfiles to avoid editing collisions.
    :END:
    You can prevent the creation of lock files by setting this
    variable to ~nil~. *Caution:* by doing so you will lose the
    benefits that this feature provides.
*** revert-without-query                                              :files:
    :PROPERTIES:
    :Description: Specify which files should be reverted without query.
    :END:
    When you edit a file that changes automatically and frequently -
    for example, a log of output from a process that continues to
    run - it may be useful for Emacs to revert the file without
    querying you. To request this behavior, wet this variable to a
    list of regular expressions. When a file name matches one of these
    regular expressions, ~find-file~ and ~revert-buffer~ will revert
    it automatically if it has changed - provided the buffer itself is
    not modified. (If you have edited the text, it would be wrong to
    discard your changes.)
*** auto-revert-verbose                                               :files:
    :PROPERTIES:
    :Description: When nil, Auto-Revert Mode does not generate any messages.
    :END:
    The Auto Revert mode do not check or revert remote files, because
    that is usually too slow. This behavior can be changed by setting
    this variable to non-nil.
*** auto-revert-use-notify                                            :files:
    :PROPERTIES:
    :Description: If non-nil Auto-Revert Mode uses file notification functions.
    :END:
    You can disable use of file notifications by customizing this
    variable to a ~nil~ value, then Emacs will check for file changes
    by polling every five seconds.
*** auto-revert-interval                                              :files:
    :PROPERTIES:
    :Description: Time, in seconds, between Auto-Revert Mode file checks.
    :END:
    If ~auto-revert-use-notify~ is set to ~nil~ then Emacs will check
    for file changes by polling every five seconds. You can change the
    polling interval through this variable.
*** auto-revert-avoid-polling                                         :files:
    :PROPERTIES:
    :Description: Non-nil to avoid polling files when notification is available.
    :END:
    By default, Auto Revert mode will poll files for changes
    periodically even when file notifications are used. Polling is
    unnecessary in many cases, and turning it off may save power by
    relying on notifications only. To do so, set this variable to
    non-nil.
*** auto-revert-notify-exclude-dir-regexp                             :files:
    :PROPERTIES:
    :Description: Regular expression of directories to be excluded from file notifications.
    :END:
    Notification is ineffective on certain file systems; mainly
    network file system on Unix-like machines, where files can be
    altered from other machines. For such file systems, polling may be
    necessary. To force polling when ~auto-revert-avoid-polling~ is
    non-nil, set this variable to match files that should be excluded
    from using notification.
*** global-auto-revert-non-file-buffers                               :files:
    :PROPERTIES:
    :Description: When nil, Global Auto-Revert Mode operates only on file-visiting buffers.
    :END:
    You can auto-revert certain non-file buffers by setting this
    variable to a non-nil value. This enables Auto Reverting for all
    types of buffers for which it is implemented (listed in the menu
    below).
*** buffer-auto-revert-by-notification                                :files:
    :PROPERTIES:
    :Description: Whether a buffer can rely on notification in Auto-Revert mode.
    :END:
    Some non-file buffers can be updated reliably by file notification
    on their default directory. The major mode can indicate this by
    setting this variable to a non-nil value in that buffer, allowing
    Auto Revert to avoid periodic polling.
*** auto-save-no-message                                         :files:echo:
    :PROPERTIES:
    :Description: Non-nil means do not print any message when auto-saving.
    :END:
    When this variable is set to ~nil~ (the default), the message
    '~Auto-saving...~' is displayed in the echo area during
    auto-saving, if any files are actually auto-saved; to disable
    these messages, customize the variable to a non-nil value.
*** auto-save-file-name-transforms                                    :files:
    :PROPERTIES:
    :Description: Transforms to apply to buffer file name before making auto-save file name.
    :END:
    This variable allows a degree of control over the auto-save file
    name. It lets you specify a series of regular expressions and
    replacements to transform the auto save file name. The default
    value puts the auto-save file for remote files into the temporary
    file directory on the local machine.
*** auto-save-interval                                                :files:
    :PROPERTIES:
    :Description: Number of input events between auto-saves.
    :END:
    Emacs auto-saves periodically based on how many characters you
    have typed since the last auto save. This variable specifies how
    many characters there are between auto-saves. By default, it
    is 300. Emacs doesn't accept values that are too small: if you
    customize this variable to a value less than 20, Emacs will behave
    as if the value is 20.
*** auto-save-timeout                                                 :files:
    :PROPERTIES:
    :Description: Number of seconds idle time before auto-save.
    :END:
    Auto-saving takes place when you stop typing for a while. By
    default, it does this after 30 seconds of idleness. To change this
    interval, customize this variable. The actual time period is
    longer if the current buffer is long; this is a heuristic which
    aims to keep out of your way when you are editing long buffers, in
    which auto-save takes an appreciable amount of time.
*** auto-save-visited-interval                                        :files:
    :PROPERTIES:
    :Description: Interval in seconds for ‘auto-save-visited-mode’.
    :END:
    You can use this variable to customize the interval between
    auto-save operations in Auto Save Visited mode; by default it's
    five seconds. ~auto-save-interval~ and ~auto-save-timeout~ have no
    effect on ~auto-save-visited-mode~.
*** delete-audo-save-files                                     :files:delete:
    A buffer's auto-save file is deleted when you save the buffer in
    its visited file. (You can inhibit this by setting this variable
    to ~nil~.)
*** auto-save-default                                                 :files:
    :PROPERTIES:
    :Description: Non-nil says by default do auto-saving of every file-visiting buffer.
    :END:
    Each time you visit a file, auto-saving is turned on for that
    file's buffer if this variable is non-nil. The default for this
...
*** auto-save-visited-interval                                        :files:
    ...
    customize this variable to change the idle time interval.
*** auto-save-list-file-prefix                                        :files:
    :PROPERTIES:
    :Description: Prefix for generating ‘auto-save-list-file-name’.
    :END:
    Emacs records information about interrupted sessions in files
    named ~.saves-<pid>-<hostname>~ in the directory
    '~/.emacs.d/auto-save-list/'. This directory is determined by this
    variable and if you set this variable to ~nil~, sessions are not
    recorded for recovery.
*** find-file-suppress-same-file-warnings                        :files:echo:
    :PROPERTIES:
    :Description: Non-nil means suppress warning messages for symlinked files.
    :END:
    Normally, if you visit a file which Emacs is already visiting
    under a different name Emacs displays a message in the echo area
    and uses the existing buffer visiting that file. You can suppress
    the message by setting this variable to a non-nil value.
*** find-file-existing-other-name                              :files:buffer:
    :PROPERTIES:
    :Description: Non-nil means find a file under alternative names, in existing buffers.
    :END:
    Normally, if you visit a file which Emacs is already visiting
    under a different name Emacs displays a message in the echo area
    and uses the existing buffer visiting that file. You can disable
    this feature entirely by setting this variable to ~nil~: then if
    you visit the same file under two different names, you can a
    separate buffer for each file name.
*** find-file-visit-truename                                          :files:
    :PROPERTIES:
    :Description: Non-nil means visiting a file uses its truename as the visited-file name.
    :END:
    If this variable is non-nil, then the file name recorded for a
    buffer is the file's truename, rather than the name you specify.
*** directory-abbrev-alist                                            :files:
    :PROPERTIES:
    :Description: Alist of abbreviations for file directories.
    :END:
    ...
    name. To do this, customize this variable. Each element in this
    list should have the form ~(<from> . <to>)~, which means to
    replace /from/ with /to/ whenever /from/ appears in a directory
    name. The /from/ string is a regular expression. It is matched
    against directory names anchored at the first character, and
    should start with '~\`~' (to support directory names with embedded
    newlines, which would defeat '~^~'). The /to/ string should be an
    ordinary absolute directory name pointing to the same
    directory. Do not use '~' to stand for a home directory in the
    /to/ string; Emacs performs these substitutions separately.
*** list-directory-brief-switches                              :files:inform:
    :PROPERTIES:
    :Description: Switches for ‘list-directory’ to pass to ‘ls’ for brief listing.
    :END:
    The text of a directory listing is mostly optained by running ~ls~
    in an inferior process. Two Emacs variables control the switches
    passed to ~ls~: this variable is a string giving the switches to
    use in brief listing ("~-CF~" by default)
*** list-directory-verbose-switches                            :files:inform:
    :PROPERTIES:
    :Description: Switches for ‘list-directory’ to pass to ‘ls’ for verbose listing.
    :END:
    The text of a directory listing is mostly optained by running ~ls~
    in an inferior process. Two Emacs variables control the switches
    passed to ~ls~: this variable is a string giving the switches to
    use in a verbose listing ("~-l~" by default).
*** delete-by-moving-to-trash                                  :files:delete:
    :PROPERTIES:
    :Description: Specifies whether to use the system’s trash can.
    :END:
    The command ~delete-directory~ prompts for a directory's name
    ...
    Bin") feature, you can make this command move the specified
    directory to the Trash instead of deleting it outright, but
    changing this variable to ~t~.
*** diff-switches                                       :files:inform:buffer:
    :PROPERTIES:
    :Description: A string or list of strings specifying switches to be passed to diff.
    :END:
    The command ~diff~ prompts for two file names, using the
    minibuffer, and displays the differences between the two files in
    a buffer named ~*diff*~. This works by running the ~diff~ program,
    using options taken from this variable. The value of this variable
    should be a string; the default is ~"-u"~ to specify an unified
    context diff.
*** compare-ignore-case                                       :window:inform:
    If this variable is non-nil, the comparison of the
    ~compare-windows~ command ignores differences in case.
*** compare-ignore-whitespace                                 :window:inform:
    If this variable is non-nil, the comparison of the
    ~compare-windows~ command ignores changes in whitespace by
    default, but a prefix argument turns that off for that single
    invocation of the command.
*** diff-update-on-the-fly                                 :mode:inform:edit:
    :PROPERTIES:
    :Description: Non-nil means hunk headers are kept up-to-date on-the-fly.
    :END:
    You can edit a Diff mode buffer like any other buffer. Whenever
    you edit a hunk, Diff mode attempts to automatically correct the
    line numbers in the hunk headers, to ensure that the patch remains
    correct, and could still be applied by ~patch~. To disable
    automatic line number correction, change this variable to ~nil~.
*** diff-refine                                          :nav:display:inform:
    :PROPERTIES:
    :Description: If non-nil, enable hunk refinement.
    :END:
    By default, Diff mode refines hunks as Emacs displays them,
    highlighting their changes with better granularity. Alternatively,
    if you set this variable to the symbol ~navigation~, Diff mode
    only refines the hunk you move to with the commands
    ~diff-hunk-next~ or ~diff-hunk-prev~.
*** diff-jump-to-old-file                                               :nav:
    :PROPERTIES:
    :Description: Non-nil means ‘diff-goto-source’ jumps to the old file.
    :END:
    Non-nil means ~diff-goto-source~ jumps to the old file. Else, it
    jumps to the new file. Default is ~nil~. Other commands might also
    reference this variable like ~diff-apply-hunk~.
*** diff-font-lock-syntax                                      :display:mode:
    :PROPERTIES:
    :Description: If non-nil, diff hunk font-lock includes source language syntax highlighting.
    :END:
    If this variable is non-nil, fragments of source in hunks are
    highlighted according to the appropriate major mode
*** delete-by-moving-to-trash                                  :delete:files:
    :PROPERTIES:
    :Description: Specifies whether to use the system’s trash can.
    :END:
    By default, Emacs deletion commands do not use the Trash. To use
    the Trash (when it is available) for common deletion commands,
    change this variable to ~t~. This affects the commands
    ~delete-file~ and ~delete-directory~, as well as the deletion
    commands in Dired.
*** auto-compression-mode                                        :files:mode:
    :PROPERTIES:
    :Description: Non-nil if Auto-Compression mode is enabled.
    :END:
    Specify whether Auto Compression mode features should be disabled
    or not. Enabled by default
*** tramp-default-method                                              :files:
    :PROPERTIES:
    :Description: Default method to use for transferring files.
    :END:
    Default method to use for transferring files. See ~tramp-methods~
    for possibilities. Also see ~tramp-default-method-alist~. Default
    value is ~"scp"~.
*** tramp-mode                                                        :files:
    :PROPERTIES:
    :Description: Whether Tramp is enabled.
    :END:
    You can entirely turn off the remote file name feature by setting
    this variable to ~nil~. Set to ~t~ by default.
*** ange-ftp-default-user                                             :files:
    :PROPERTIES:
    :Description: User name to use when none is specified in a file name.
    :END:
    When the Ange-FTP package is used, Emacs logs in through FTP using
    the name /user/, if that is specified in the remote file name. If
    /user/ is unspecified, Emacs logs in using your user name on the
    local system; but if you set this variable to a string, that
    string is used instead.
*** ange-ftp-make-backup-files                                        :files:
    :PROPERTIES:
    :Description: Non-nil means make backup files for "magic" remote files.
    :END:
    For performance reasons, Emacs does not make backup files for
    files accessed via FTP by default. To make it do so, change this
    variable to a non-nil value.
*** ange-ftp-generate-anonymous-password                              :files:
    :PROPERTIES:
    :Description: If t, use value of ‘user-mail-address’ as password for anonymous FTP.
    :END:
    To visit files accessible by anonymous FTP, you use special user
    names '~anonymous~' or '~ftp~'. Passwords for these user names are
    handled specially. This variable controls what happens: if the
    value of this variable is a string, then that string is used as
    the password; if non-nil (the default), then the value of
    ~user-mail-address~ is used; if ~nil~, then Emacs prompts you for
    a password as usual.
*** user-mail-address                                                :inform:
    :PROPERTIES:
    :Description: The email address of the current user.
    :END:
    The email address of the current user. This defaults to either:
    the value of EMAIL environment variable; or user@host, using
    '~user-login-name~' and '~mail-host-address~' (or
    '~system-name~').
*** ange-ftp-gateway-host                                            :inform:
    :PROPERTIES:
    :Description: Name of host to use as gateway machine when local FTP isn’t possible.
    :END:
    Name of host to use as gateway machine when local FTP isn't
    possible.
*** ange-ftp-smart-gateway                                           :inform:
    :PROPERTIES:
    :Description: Non-nil says the FTP gateway (proxy) or gateway FTP program is smart.
    :END:
    Non-nil says the FTP gateway (proxy) or gateway FTP program is
    smart.
*** recentf-list                                               :files:inform:
    :PROPERTIES:
    :Description: ("/usr/share/emacs/27.1/lisp/loaddefs.el" "/home/occhio/Documents/Productivity/org/org-manual/emacs-manual.org" "/home/occhio/Documents/Productivity/org/org-manual/kio-big-manual.org" "/home/occhio/Documents/Productivity/org/org-manual/commands.csv" "/home/occhio/.emacs" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-24-00.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-31-05.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-31-04.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-31-03.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-31-02.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-31-01.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-31-00.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-29-00.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-27-00.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-26-07.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-26-06.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-26-05.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-26-04.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-26-03.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-26-02.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-26-01.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-26-00.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-25-02.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-25-01.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-25-00.jpg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-24-04.jpeg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-24-03.jpeg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-24-02.jpeg" "/home/occhio/Pictures/Ellis/Fennec/2020/10-October/fennec-2020-10-24-01.jpg")
    :END:
    List of recently opened files
*** image-auto-resize                                         :files:display:
    :PROPERTIES:
    :Description: Non-nil to resize the image upon first display.
    :END:
    Non-nil to resize the image upon first display. It's value should
    be one of the following:
    - ~nil~ :: meaning no resizing.
    - ~t~ :: meaning to fit the image to the window height and width.
    - '~fit-height~' :: meaning to fit the image to the window
      height.
    - '~fit-width~' :: meaning to fit the image to the window width.
    - A number :: which is a scale factor (the default size is 1).

    Default value is ~t~
*** image-auto-resize-on-window-resize                        :files:display:
    :PROPERTIES:
    :Description: Non-nil to resize the image whenever the window’s dimensions change.
    :END:
    Non-nil to resize the image whenever the window's dimensions
    change. This will always keep the image fit to the window. When
    non-nil, the value should be a number of seconds to wait before
    resizing according to the value specified in
    '~image-auto-resize~'. Default value is 1.
*** image-animate-loop                                        :files:display:
    :PROPERTIES:
    :Description: Non-nil means animated images loop forever, rather than playing once.
    :END:
    If an image can be animated and you opted to start the animation
    then by default the animation plays once, unless this variable is
    non-nil.
*** imagemagick-enabled-types                          :files:display:inform:
    :PROPERTIES:
    :Description: List of ImageMagick types to treat as images.
    :END:
    This variable lists the image types that Emacs may render using
    ImageMagick; each element in the list should be an internal
    ImageMagick name for an image type, as a symbol or an equivalent
    string. To enable ImageMagick for all possible image types change
    this variable to ~t~.
*** imagemagick-types-inhibit                          :files:display:inform:
    :PROPERTIES:
    :Description: List of ImageMagick types that should never be treated as images.
    :END:
    This variable lists the image types which should never be rendered
    using ImageMagick, regardless of the value of
    ~imagemagick-enagled-types~ (the default list includes types like
    ~C~ and ~HTML~, which ImageMagick can render as an image but Emacs
    should not). To disable ImageMagick entirely, change this variable
    to ~t~.
*** image-use-external-converter                              :files:display:
    :PROPERTIES:
    :Description: If non-nil, ‘create-image’ will use external converters for exotic formats.
    :END:
    If Emacs doesn't have native support for image format in question,
    and this variable is non-nil, Emacs will try to determine whether
    there are external utilities that can be used to transform the
    image in question to PNG before displaying. GraphicsMagick,
    ImageMagick and ~ffmpeg~ are currently supported for image
    conversions.
** Using Multiple Buffers
*** major-mode                                                  :mode:buffer:
    :PROPERTIES:
    :Description: Symbol for current buffer’s major mode.
    :END:
    If you specify a buffer that does not exist, ~C-x b~ creates
    a new, empty buffer that is not visiting any file, and selects it
    for editing. The default value of this variable determines the new
    buffer's major mode; The default value is Fundamental mode.
*** buffer-read-only                                            :buffer:edit:
    :PROPERTIES:
    :Description: Non-nil if this buffer is read-only.
    :END:
    The command ~C-x C-q~ (~read-only-mode~) makes a read-only buffer
    writable, and makes a writable buffer read-only. This works by
    setting this variable, which has a local value in each buffer and
    makes the buffer read-only if its value is non-nil.
*** view-read-only                                                   :buffer:
    :PROPERTIES:
    :Description: Non-nil means buffers visiting files read-only do so in view mode.
    :END:
    If you change this variable to a non-nil value, making the buffer
    read only with ~C-x C-q~ also enables View mode in the buffer.
*** midnight-mode                                                   :startup:
    :PROPERTIES:
    :Description: Non-nil if Midnight mode is enabled.
    :END:
    To enable Midnight mode, you can use the Customization buffer to
    set this variable to ~t~.
*** global-auto-revert-non-file-buffers                 :startup:buffer:edit:
    :PROPERTIES:
    :Description: When nil, Global Auto-Revert Mode operates only on file-visiting buffers.
    :END:
    Global Auto Revert mode applies to the ~*Buffer List*~ buffer only
    if this variable is non-nil.
*** uniquify-buffer-name-style                                       :buffer:
    :PROPERTIES:
    :Description: How to construct unique buffer names for files with the same base name.
    :END:
    You can choose from several different styles for constructing
    unique buffer names, by customizing this option.

    Example files to make buffers unique:
    - ~/u/rms/tmp/Makefile~
    - ~/src/expl/tmp/Makefile~
    - ~/usr/projects/zaphod/Makefile~

    Here are some of
    the possible values:
    - ~forward~ :: This naming methoc includes part of the file's
      directory name at the beginning of the buffer name; Using this
      method, buffers visiting example files would be named
      '~rms/tmp/Makefile~', '~expl/tmp/Makefile~' and
      '~zaphod/Makefile~'.
    - ~post-forward~ :: This naming method would call the example file
      buffers '~Makefile|rms/tmp~', '~Makefile|expl/tmp~ and
      '~Makefile|zaphod~'.
    - ~post-forward-angle-brackets~ :: Like ~post-forward~, except
      that it encloses the unique path in angle brackets. This is the
      default. '~Makefile<rms/tmp>~', '~Makefile<expl/tmp>~ and
      '~Makefile<zaphod>~'.
    - ~reverse~ :: Example buffer names, '~Makefile\tmp\rms~',
      '~Makefile\tmp\expl~' and '~Makefile\zaphod~'
    - ~nil~ :: examples, '~Makefile~', '~Makefile<2>~', and
      '~Makefile<3>~'
*** icomplete-mode                                            :complete:mode:
    :PROPERTIES:
    :Description: Non-nil if Icomplete mode is enabled.
    :END:
    To enable Icomplete mode you can customize this variable to
    ~t~. ~nil~ by default.
*** fido-mode                                                 :complete:mode:
    :PROPERTIES:
    :Description: Non-nil if Fido mode is enabled.
    :END:
    To enable Fido mode, customize this variable to ~t~. ~nil~ by
    default.
** Multiple Windows
*** split-window-keep-point                                          :window:
    :PROPERTIES:
    :Description: If non-nil, C-x 2 preserves point in the new window.
    :END:
    If you change this variable to ~nil~, ~C-x 2~ instead adjusts the
    portion of the buffer displayed by the two windows, as well as the
    value of point in each window, in order to keep the text on the
    screen as close as possible to what it was before; Furthermore, if
    point was in the lower half of the original window, the bottom
    window is selected instead of the upper one.
*** truncate-partial-width-windows                                   :window:
    :PROPERTIES:
    :Description: Non-nil means truncate lines in windows narrower than the frame.
    :END:
    When you split a window with ~C-x 3~, each resulting window
    occupies less than the full width of the frame. If it becomes too
    narrow, the buffer may be difficult to read if continuation lines
    are in use. Therefore, Emacs automatically switches to line
    truncation if the window width becomes narrower than 50
    columns. This truncation is controlled by this variable. If the
    value of this variable is a positive integer (the default is 50),
    the specifies the minimum total width for a partial-width window
    before automatic line truncation occurs; if the value is ~nil~,
    automatic line truncation is disabled; and for any other non-nil
    value, Emacs truncates lines in every partial-width window
    regardless of its width.
*** window-total-width                                               :window:
    The total width of a window is in column units as reported by this
    variable, it includes the fringes, the continuation and truncation
    glyphs, the margins and the scroll bar.
*** window-resize-pixelwise                                          :window:
    :PROPERTIES:
    :Description: Non-nil means resize windows pixelwise.
    :END:
    ...
    default font size of the fram. That might subdivide the screen
    estate unevenly between the resulting windows. If you set this
    variable to a non-nil value, Emacs will give each window the same
    number of pixels (give or take one pixel if the enitial dimension
    was an odd number of pixels). Note that when a frame's pixel size
    is not a multiple of the frame's character size, at least one
    window may get resized pixelwise even if this option is ~nil~.
*** mouse-autoselect-window                                          :window:
    :PROPERTIES:
    :Description: Non-nil means autoselect window with mouse pointer.
    :END:
    If you set this variable to a non-nil value, moving the mouse over
    a different window selects that window. This feature is off by
    default.
*** window-min-height                                                :window:
    :PROPERTIES:
    :Description: The minimum total height, in lines, of any window.
    :END:
    Cannot reduce a window height below the number of lines defined in
    this variable. The default is 4.
*** window-min-width                                                 :window:
    :PROPERTIES:
    :Description: The minimum total width, in columns, of any window.
    :END:
    Cannot reduce a window width below the number of columns defined
    in this variable. The default is 10.
*** display-buffer-alist                                      :window:buffer:
    :PROPERTIES:
    :Description: Alist of user-defined conditional actions for ‘display-buffer’.
    :END:
    Alist of user-defined conditional actions for ~display-buffer~. By
    default this is nil.
*** display-buffer-base-action                                       :window:
    :PROPERTIES:
    :Description: User-specified default action for ‘display-buffer’.
    :END:
    User-specified default action for ~display-buffer~
*** split-height-threshold                                           :window:
    :PROPERTIES:
    :Description: Minimum height for splitting windows sensibly.
    :END:
    Minimum height for splitting windows sensibly.
*** split-width-threshold                                            :window:
    :PROPERTIES:
    :Description: Minimum width for splitting windows sensibly.
    :END:
    Minimum width for splitting windows sensibly.
*** temp-buffer-max-height                                           :window:
    :PROPERTIES:
    :Description: Maximum height of a window displaying a temporary buffer.
    :END:
    The maximum height of windows resized by ~temp-buffer-resize-mode~
    can be controlled by customizing this variable, and cannot exceed
    the size of the containing frame.
*** temp-buffer-max-width                                            :window:
    :PROPERTIES:
    :Description: Maximum width of a window displaying a temporary buffer.
    :END:
    The maximum width of windows resized by ~temp-buffer-resize-mode~
    can be controlled by customizing this variable, and cannot exceed
    the size of the containing frame.
*** winner-mode                                         :window:undo:display:
    :PROPERTIES:
    :Description: Non-nil if Winner mode is enabled.
    :END:
    Can customize this variable to enable or disable Winner mode
*** winner-dont-bind-my-keys                                    :window:keys:
    To prevent Winner mode from binding ~C-c <left>~ and ~C-c
    <right>~, you can customize this variable to a non-nil value.
*** winner-ring-size                                            :window:hist:
    By default, Winner mode stores a maximum of 200 windows
    configurations per frame, but you can change that by modifying
    this variable.
*** winner-boring-buffers                                :window:hist:buffer:
    If there are some buffers whose windows you wouldn't want Winner
    mode to restore, add their names to this list variable.
*** winner-boring-buffers-regexp                         :window:hist:buffer:
    If there are some buffers whose name you wouldn't want Winner mode
    to restore, add a regexp that would catch those buffer names to
    this regexp variable
*** tab-line-tabs-function                            :window:buffer:display:
    You can customize this variable to define the prefered contents of
    the tab line. By default, it displays all buffers previously
    visited in the window, as described above. But you can also set it
    to display a list of buffers with the same major mode as the
    current buffer, or to display buffers grouped by their major mode,
    where clicking on the mode name in the first tab displays a list
...
*** before-save-hook
    This is the hook which fires all functions in the list just before
    saving a file.
** Using Multiple Buffers
*** kill-buffer-hook
    If you want to do something special every time a buffer is killed,
    you can add hook functions to this hook
*** midnight-hook
    Commands that get run at midnight everyday if Midnight mode is
    enabled.
*** clone-indirect-buffer-hook
    The functions ~clone-indirect-buffer~ and
    ~clone-indirect-buffer-other-window~ both run this hook after
...
*** vertical-border
    ...
    vertical divider which is drawn using this face.
* .emacs additions
** Registers
*** Saving Text in Registers
...
*** Filesets
    ...
    them as a fileset. This lets you perform certain operations, such
    as visiting, ~query-replace~, and shell commands on all the files
    at once. To make use of filesets, you must first add the
    expression:
    #+BEGIN_SRC emacs-lisp
      (filesets-init)
    #+END_SRC
    This adds a '~Filesets~' sub-menu to the menu bar's '~File~'
    menu.

** Using Multiple Buffers
*** Change Fido defaults
    A noteworthy aspect of Fido mode is that ~flex~ is used as the
    default completion style. To change this, add the following to
...
*** Customize ~display-buffer~
    You can tell Emacs not to skip first step of ~display-buffer~ by
    adding a regular expression matching the buffer's name together
    with a reference to the ~display-buffer-same-window~ action
    function to the option ~display-buffer-alist~. For example, to
    display the buffer ~*scratch*~ preferably in the selected window
    write:
    #+BEGIN_SRC emacs-lisp
      (customize-set-variable
       'display-buffer-alist
       '("\\*scratech\\*" (display-buffer-same-window)))
    #+END_SRC

*** Further Customization
    Windows on other frames are also reusable if you use the
    corresponding ~reusable-frames~ action alist entry.

    You can also enable creating a new frame and displaying the buffer
    there by chanigng the value of the option
    ~display-buffer-base-action~ as follows:
    #+BEGIN_SRC emacs-lisp
      (customize-set-variable
    ...
    This customization will also try to search for a reusable window
    on all visible or iconified frames as described earlier.
*** Temp buffer display
    If you prefer Emacs to display a temporary buffer in a different
    fashion, customize the variable ~display-buffer-alist~
    appropriately. For example, to display ~*Completions*~ always
    below the selected window, use the following form in your
    initialization file:
    #+BEGIN_SRC emacs-lisp
      (customize-set-variable
       'display-buffer-alist
...
** File Handling
*** mode
    Should probably be the first local variable defined. Defines what
    major mode should be used to interpret the file. Helps to use the
    comment syntax of that mode to comment out wherever you define
    this file variable.

    A reminder of what that syntax looks like when you define this
    file variable as the first line of the file:
    #+BEGIN_SRC emacs-lisp
      ;; -*- mode: Lisp; -*-
    #+END_SRC

*** Local Variables
    This is a variable that can be put near the end of a file. Again,
    best to use the comment syntax of whatever mode is defined to
    comment out any file variables. Reminder of basic syntax:
    #+BEGIN_SRC emacs-lisp
      Local Variables:
      mode: c
      comment-column: 0
...
*** Time-stamp
    You can arrange to put a time stamp in a file, so that it is
    updated automatically each time you edit and save the file. The
    time stamp must be in the first eight lines of the file, and you
    should insert it like this:
    #+BEGIN_SRC emacs-lisp
      ;; Time-stamp: <>
    #+END_SRC
...
** File Handling
*** time-stamp
    Group of variables related to customizing everything concerning
    the time stamp feature
*** Archive
    Details of the program names and their options as they relate to
    Archive mode can be set in this customize group
** Using Multiple Buffers
*** bs
    Custom group to customize all things related to command
    ~bs-show~.
* Niche Modes
** Registers
*** Bookmark Menu
...
*** Ediff
    [[https://www.gnu.org/software/emacs/manual/html_node/ediff/index.html#Top][manual]]
**** Description
     A much more sophisticated alternative to Diff mode.
**** Enter Mode
     ~M-x ediff~
**** Find Commands
     ~C-h m~
*** Smerge mode
**** Description
     A minor mode for editing output from the ~diff3~ program. This is
     typically the result of a failed merge from a version control
     system update outside VC, due to conflicting changes to a
     file. Smerge mode provides commands to resolve conflicts by
...
**** Find Commands
     ~C-h m~
*** Emerge
    [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Emerge.html][manual]]
**** Description
     Provides a powerful interface for merging files.
**** Enter Mode
     ~M-x emerge-files~, ~M-x emerge-files-with-ancestor~, ~M-x
     emerge-buffers~, ~M-x emerge-buffers-with-ancestor~
**** Find Commands
     ~C-h m~
** Using Multiple Buffers
*** MSB mode
**** Description
     "MSB" stands for "mouse select buffer". MSB mode provides a
     different and customizable mouse buffer menu which you may
     prefer. It replaces the ~mouse-buffer-menu~ commands, normally
     bound to ~C-Down-mouse-1~ and ~C-F10~, with its own commands, and
...
** File Handling
*** ask-user-about-lock
    When Emacs detects a collision, it asks you what to do, by calling
    this Lisp function. You can redefine this function for the sake of
    customization.
*** time-stamp
    Function which automatically updates the time stamp in a file with
    the current date and time.
*** make-auto-save-file-name
    Return file name to use for auto-saves of current buffer
*** auto-save-file-name-p
    Return non-nil if given filename can be yielded by
    ~make-auto-save-file-name~.
** Multiple Windows
